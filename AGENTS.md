...
- After **every** `git worktree add`, initialize that checkout's submodules with the checked-in initializer — never by copying `.git` metadata:
  ...
  It runs native `git submodule update --init --recursive` for the submodules that are missing and validates the ones that already exist, so a repeated call keeps each submodule's own gitdir, index, config and HEAD (an intentional pin difference is reported, never reset) and shares nothing with main. Every initialized path is validated first, and the command writes exactly one JSON object to stdout. Exit `0` means the checkout is valid and its submodules were validated or initialized; exit `2` is an invalid invocation; exit `1` is a Git, safety or operational failure whose reason is on stderr, and whose `refusal` object says what the failure left behind: `init.refuse.preflight` left the checkout exactly as it was — either this run attempted no mutation at all (`initialized_paths: null`) or it ran the native update and completed no submodule (`initialized_paths: []`); `init.refuse.partial` means native Git had already initialized at least one submodule, and `initialized_paths` lists those paths (≥ 1, re-observed read-only) — initialization is not transactional and nothing is rolled back, so a re-run validates the initialized subset and finishes the rest; `initialized_paths` is also `null` when the checkout could not be re-read (an unknown state, never an empty one). Keep full recursive submodules; never disable recursion globally.
