...
| [daemon-api-workspace-write-architecture.md](daemon-api-workspace-write-architecture.md) | V1.15 | Workspace-write architecture (D1–D7). **Route list superseded by V1.20** compass §3. | Active (partial) |
| [schemas-wire-platform-sync-boundary.md](schemas-wire-platform-sync-boundary.md) | V1.4 WS5 | Which `schemas/` types sync to `@42ch/nexus-contracts` / platform vs `local/` Rust only. | Active |
