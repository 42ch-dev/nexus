...
| **`nexus-moment-context-assembly`** | `Moment` | Per-moment, pre-session context aggregation. The local `assemble_moment` path can aggregate Creator memory, narrative state, World KB assets, and User knowledge. V1.26 ships two CLI-local paths: `assemble-local` for narrower Stage-0/TwoStage assembly, and `assemble-moment` for local four-domain Moment assembly with persistent narrative / World KB stores plus an in-memory User knowledge slice. Optional `cloud-stage` may merge future platform context; direct platform cloud assembly remains deferred. | Only with `cloud-stage` |
...
- **Shipped local four-domain Moment path (V1.26):** `assemble_moment` depends on `nexus-creator-memory`, `nexus-narrative`, `nexus-kb`, `nexus-knowledge`, and `nexus-contracts`. `nexus42 platform context assemble-moment` calls it in-process with local persistent narrative / World KB stores; User knowledge remains an in-memory slice until a persistent User knowledge store is introduced.
- **Shipped Stage-0/TwoStage CLI path:** `nexus42 platform context assemble-local` uses `Stage0Assembly` / `TwoStageAssembly` in-process over Creator SOUL, Creator long-term memory, optional local fragment keywords, and a future direct platform Stage-1 response. This path remains narrower than full four-domain `assemble_moment`.
- **Deferred platform cloud path:** `nexus42 platform context assemble` is not yet available as direct platform cloud assembly and should guide users to the shipped local paths.
