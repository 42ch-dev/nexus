...
## 4. Technical Freeze (V1.0)

This section captures frozen technical decisions from the V1.0 Context Assembly specification. All implementation must conform to these constraints.

### 4.1 Core Shape

The V1.0 Context Assembly architecture uses:

- **Agentic routing**: An agent (or equivalent orchestration layer) routes the query to the appropriate retrieval backends based on query intent and the input types requested.
- **HybridRAG**: Combines structured graph retrieval (Neo4j) with vector similarity search (pgvector) and relational metadata queries (Postgres). Results are merged and sorted before returning a unified snapshot.
- **All-TypeScript implementation**: The Context Assembly service, including all retrieval logic, query façade, and orchestration, is implemented entirely in TypeScript. No Rust crate for context assembly exists or will be created.
...
A **GraphQL or equivalent read-only façade** serves as the single aggregation layer. All context retrieval goes through this façade — consumers do not query Neo4j, Postgres, or pgvector directly.

- The façade is **read-only**: it never mutates world state.
- It accepts the `ContextAssembleRequestV1` shape and returns `ContextAssembleResponseV1`.
- The CLI accesses this via `POST /v1/local/context/assemble`, proxied by `nexus42d`.

### 4.4 Sorting Strategy

**V1.0 minimal sorting pipeline** (frozen — do not enhance in V1.0 scope):

1. **Metadata filter**: Apply request-level filters (`memory_kinds`, `max_timeline_events`, `max_story_summaries`).
2. **Vector top-k**: Retrieve top-k embeddings from pgvector based on query intent.
3. **Graph expansion**: Traverse Neo4j relationships to include connected entities (KB → related KBs, timeline → adjacent events).
4. **Rule merge**: Merge results from all sources, deduplicate, apply relevance rules.
5. **Stable sort**: Final output has deterministic order (same state → same order).

**V1.0 does NOT require** (deferred to V1.1/V1.2/Backlog):

- LLM-as-judge reranking (using an LLM to score/rank results)
- ColBERT or cross-encoder reranking
- Declarative query DSL for advanced filtering/sorting
...
- **V1.1 safety constraints** (see §9): File size limit, path traversal validation, UTF-8 truncation safety.
...
#### 8.3.1 Request: `ContextAssembleRequestV1`
...
#### 8.3.2 Response: `ContextAssembleResponseV1`
...
### 8.4 Bundle Metadata Integration
...
## 9. V1.1 Enhancement Constraints

This section defines **design constraints** for V1.1 enhancements to the CLI-side summary generator. These are frozen design-level specifications — implementation details (code blocks, specific function signatures) belong in the execution plan, not here.

### 9.1 Summary Generator Safety

The following safety constraints apply to the V1.1 summary generator (see §8.1 Summary Generation). V1.0 uses basic extraction without these constraints; V1.1 adds them as mandatory.

#### 9.1.1 File Size Limit (CTX-R2)

- **Constraint**: The summary generator must enforce a maximum file size for individual files it processes. Files exceeding the limit are skipped with a warning logged.
- **Interface**: `SummaryConfig.max_file_size` — a `u64` value in bytes.
- **Default**: 10 MB (10 × 1024 × 1024 bytes).
- **Environment override**: `NEXUS_CONTEXT_MAX_FILE_SIZE` — if set and parseable as `u64`, overrides the default.
- **Acceptance criteria**:
  - Files exceeding `max_file_size` are skipped (not read), and a warning is logged with path, actual size, and limit.
  - Files at or under `max_file_size` are processed normally.
  - The limit is configurable via `SummaryConfig` struct and env var.

#### 9.1.2 Path Traversal Validation (CTX-R3)

- **Constraint**: The summary generator's recursive directory scanner must validate that every resolved path stays within the configured base directory. This prevents path traversal attacks (e.g., `../../etc/passwd`) and symlink escape.
- **Interface**: `validate_path_within_base(path, base) -> Result<PathBuf, SummaryError>` — canonicalizes both `path` and `base`, then checks that the canonicalized `path` starts with the canonicalized `base`.
- **Symlink escape detection**: Because canonicalization resolves symlinks, any symlink pointing outside the base directory is detected as a traversal violation.
- **Error type**: `SummaryError::PathTraversal { path, base }` — returned when the resolved path is outside the base.
- **Acceptance criteria**:
  - Paths using `..` to escape the base directory are rejected with `PathTraversal` error.
  - Symlinks pointing outside the base directory are rejected.
  - Paths legitimately within the base directory (including subdirectories) are accepted.
  - The validation is applied at every level of recursive scanning.

#### 9.1.3 UTF-8 Truncation Safety (CTX-R5)

- **Constraint**: When truncating text to fit summary size limits (e.g., the opening excerpt in §8.1), the truncation must occur at valid UTF-8 character boundaries. Splitting a multi-byte character (CJK 3-byte sequences, emoji 4-byte sequences) produces invalid UTF-8 and must be prevented.
- **Interface**: `truncate_at_char_boundary(s: &str, max_bytes: usize) -> &str` — returns the longest prefix of `s` that is at most `max_bytes` bytes and ends on a valid character boundary. If `s.len() <= max_bytes`, returns `s` unchanged.
- **Acceptance criteria**:
  - ASCII text truncates at exact byte position when on a boundary.
  - CJK text (3 bytes per character) truncates at the nearest preceding character boundary, not mid-character.
  - Emoji text (4+ bytes per codepoint) truncates without splitting a codepoint.
  - Mixed ASCII/CJK/emoji text handles all boundary cases correctly.
  - Empty string returns empty string.
  - `max_bytes >= s.len()` returns `s` unchanged.
  - Result always passes `is_char_boundary()` check.
...
## 11. Task Breakdown (CLI-Side Implementation)
...
## 12. Dependency on Sync Contract
...
## 14. Implementation Effort (Agent-Oriented)
