...
| ---------------- | ------ | --------------------------------------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| User             | §5.1   | ❌ Not implemented                             | ❌ Not in schemas                | **Gap** — no User aggregate in domain or contracts                                                                          |
| Creator          | §5.2   | ✅ `creator.rs`                                | ✅ `creator.rs`                  | **Aligned**                                                                                                                 |
| Pairing          | §5.2A  | ✅ `pairing.rs`                                | ✅ `pairing.rs`                  | **Aligned**                                                                                                                 |
| World            | §5.3   | ✅ `world.rs`                                  | ✅ `world.rs`                    | **Partial** — see WorldStatus enum mismatch                                                                                 |
| WorldMembership  | §5.4   | ✅ `world_membership.rs`                       | ✅ `world_membership.rs`         | **Partial** — see MembershipStatus enum mismatch                                                                            |
...
| ForkBranch       | §5.7   | ✅ `fork_branch.rs`                            | ✅ `fork_branch.rs`              | **Partial** — spec has `parent_branch_id` and `forked_from_event_id`; contracts use `forked_from_event_id` only; see detail |
...
**Summary**: 11 of 17 aggregates have domain implementations. 6 are missing domain logic (User, SyncCommand, DeltaBundle/Delta, OutboxEntry, WorkspaceBinding, AgentProfile). Of the 11 implemented, 9 are fully aligned and 2 have enum mismatches.
...
| ------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------------------------ | ------------------------------------------- |
| User                | `account_status`        | `active, suspended, deleted`                                                                                          | ❌ No User aggregate                                                           | ❌ Not in contracts       | N/A                                         |
...
| User                | `subscription_tier`     | `free, pro, studio, enterprise`                                                                                       | ❌ No User aggregate                                                           | ❌ Not in contracts       | N/A                                         |
| **World**           | `**status`**            | `**active, paused, archived**`                                                                                        | `**Active, Archived, Frozen**`                                                | `String`                 | **❌ `Frozen` ≠ `Paused`**                   |
...
| **WorldMembership** | `**membership_status`** | `**active, invited, suspended, removed**`                                                                             | `**Active, Suspended, Left**`                                                 | `String`                 | **❌ `Left` ≠ `Removed`; `invited` missing** |
...
1. **2 enum value mismatches** (Critical):
  - `WorldStatus`: Spec says `paused`, implementation uses `Frozen`
  - `MembershipStatus`: Spec says `removed` + `invited`, implementation uses `Left` (missing `invited`)
2. **28+ fields typed as `String` in contracts that should be enum-typed**: The generated contract types use `String` for nearly all enum fields. Only 6 enums are properly typed (`BlockType`, `MemoryType`, `ManuscriptPhase`, `TimePolicy`, `Visibility`, `BundleType`). This creates a validation gap — the domain layer has the correct Rust enums, but wire contracts don't enforce them.
3. **6 aggregates without domain logic**: User, SyncCommand, DeltaBundle/Delta, OutboxEntry, WorkspaceBinding, AgentProfile have no `nexus-domain` module. SyncCommand/DeltaBundle/Delta/OutboxEntry are partially covered by `nexus-sync`, but with a different architecture (builder pattern vs domain aggregate).
