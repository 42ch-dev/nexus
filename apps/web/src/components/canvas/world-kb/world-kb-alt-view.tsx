/**
 * World KB non-spatial alternate view — sortable entity list (V1.73 P0 A6/A8).
 *
 * Per canvas-strategy-surface.md Draft §4.4 + compass §1.1 A6, this is a
 * complete accessible equivalent to the canvas graph: the same write surface
 * (inspect → edit/decide → submit) laid out as a sortable list. Columns:
 * Name / BlockType / Lifecycle / Source anchors / Updated. Rows are
 * keyboard-focusable; Enter/Space opens the matching inspector.
 *
 * Virtualization: rows are windowed by scroll position with a fixed row height
 * so worlds with 100+ entities stay responsive — no new dependency.
 *
 * simplify: fixed 44px row height windowing. Upgrade to react-window (or
 * @tanstack/react-virtual) only if row height becomes dynamic or counts exceed
 * several thousand; the V1.73 entity volume is in the low hundreds.
 */
...
const ROW_HEIGHT = 44;
/** Render a window of rows around the viewport; large enough that scrolling never shows a gap. */
const OVERSCAN = 8;
...
export interface WorldKbAltViewProps {
  ...
  /** Currently selected node id (entity:<id> or candidate:<id>). */
  selectedId: string | null;
  onSelect: (node: WorldKbNodeData) => void;
}

export function WorldKbAltView({ nodes, selectedId, onSelect }: WorldKbAltViewProps) {
  ...
  return (
    ...
    >
      ...
      <p className="border-t border-gray-alpha-200 px-3 py-2 text-label-12 text-gray-700">
        {sorted.length} {sorted.length === 1 ? 'entry' : 'entries'} · list view (keyboard /
        screen-reader primary)
      </p>
    </section>
  );
}
