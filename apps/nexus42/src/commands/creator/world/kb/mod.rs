//! World KB author surface — the single retained `creator world kb`
//! implementation (v1.193 P2-T13; the `basic-cli`/`legacy-cli` router and
//! the `slim`/`legacy_impl` split are gone).
//!
//! The clap declarations and the human/JSON formatting for
//! `creator world kb graph` and `creator world kb entity patch` live in this
//! module; only the service invocation lives in [`service`]. The local-DB
//! author leaves (`list`/`show`/`edit`/`delete`/`pending`/`adopt`/`reject`)
//! and the pack I/O surface ([`pack`]) are the rest of the author CLI.
//!
//! Both the direct-core leaves and the local leaves are admitted by
//! [`crate::core::require_materialized_workspace`] before the workspace pool
...
/// `creator world kb graph` flags.
///
/// Flattened into the `WorldKbCommand::Graph` variant so the declaration
/// exists once while the CLI shape stays
...
pub async fn run(cmd: WorldKbCommand, config: &CliConfig) -> Result<()> {
    match cmd {
        ...
        WorldKbCommand::Graph { args } => run_graph(args, config).await,
    ...
    match row {
        ...
        Some(None) => Err(CliError::Other(format!(
            "Cannot write reject audit log: work '{wid}' has no story_ref (work_ref). \
             Set the Work's story_ref before rejecting a candidate."
        ))),
        Some(Some(story_ref)) => Ok(Some(story_ref)),
    }
}

/// Run `creator world kb graph` (direct core read).
