...
pub struct CliConfig {
    ...
    /// Written by the runtime when degradation occurs; read-only for CLI display.
    ///
    /// v1.193 P2-T13: `cli`-only (the sole remaining producer/consumer); the
    /// Connect cohort keeps `chrono` out of its tree, and serde ignores this key
    /// when the field is absent (no `deny_unknown_fields`), so an existing
    /// config.toml still loads unchanged.
    #[cfg(feature = "cli")]
    #[serde(skip_serializing_if = "Option::is_none")]
