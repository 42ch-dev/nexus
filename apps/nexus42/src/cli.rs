...
pub enum Commands {
    ...
    /// Connect Host (DF-72 N-C1) — peer surface for third-party reasoners
    ///
    ...
    /// `InvokeHandlerV2`). Serves `upsert` / `promote` / `relate` with
    /// fail-closed world scoping; every other op is refused. Compiled only
