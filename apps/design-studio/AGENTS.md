...
- **Annotation placement:** the `// transitional` marker must appear on the **same line as the `from` clause** of the import (inline annotation). For single-line imports this means the same line as the import statement; for multiline imports it must be on the `from` clause line. Placing the annotation on the line above the import is not valid under this convention. The guardrail in `tooling/check-ui-guardrails.sh` enforces this by checking the `from` clause line itself for the `transitional` keyword.
