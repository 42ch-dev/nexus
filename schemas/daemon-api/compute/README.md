# Daemon API — Compute Module ABI Schemas

JSON Schemas for the **WASM compute module ABI envelopes** consumed by external compute modules (and, in future, the WebApp/Web-UI Daemon API client). These are cross-language contracts (Rust host ↔ `wasm32-unknown-unknown` module), so they live under `schemas/` and run through codegen rather than as hand-written local types.
