# Daemon API — Orchestration Sessions READ Schemas

JSON Schemas for the **Orchestration Engine Sessions** daemon endpoints under `/v1/daemon/orchestration/sessions`. These are cross-language contracts consumed by future WebApp/Web-UI clients.
...
|----------|-------------|
| `GET /v1/daemon/orchestration/sessions` | `list-sessions-query.schema.json`, `list-sessions-response.schema.json`, `session-summary.schema.json` |
| `GET /v1/daemon/orchestration/sessions/{id}` | `session-detail-response.schema.json`, `session-summary.schema.json` |
