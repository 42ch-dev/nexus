# Daemon API — Common Shared Envelopes

Cross-resource shared envelopes served under `/v1/daemon/*`. These are
cross-language contracts consumed by future WebApp/Web-UI clients (and the
`@42ch/nexus-contracts` npm package).

V1.64 (F-E1) — introduced for the canonical Daemon API error envelope.

## Schemas

| Schema | Purpose |
|--------|---------|
| `error-response.schema.json` | Canonical `ErrorResponse` detail (`{ code, message, details? }`). The daemon wraps this as `{ success: false, error: ErrorResponse, request_id?: string }` on the wire. All Daemon API failure paths converge on this shape. |
...
and is `$ref`-shared by all cursor-paginated Daemon API list responses. Promoting
