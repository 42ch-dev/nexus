...
/**
 * Response body for POST /v1/daemon/memory/soul/reflect. Reports the whole-Creator SOUL narrative cache state, stale metadata, current counts, and insufficient-data thresholds.
 */
export interface SoulNarrativeResponse {
  ...
  /**
   * Current whole-Creator fragment count used for stale and insufficient-data decisions.
   */
  current_fragment_count: number;
  /**
   * Current distinct keyword count across all Creator fragments.
   */
  current_distinct_keyword_count: number;
  /**
   * Minimum fragment count required before synthesis is allowed. Exposed so clients can render threshold copy and tests.
   */
  min_fragment_count: 10;
  /**
   * Minimum distinct keyword count required before synthesis is allowed. Exposed so clients can render threshold copy and tests.
   */
