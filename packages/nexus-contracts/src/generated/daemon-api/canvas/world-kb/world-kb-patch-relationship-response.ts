...
/**
 * Success response for POST /v1/daemon/worlds/{world_id}/kb/patch-relationship (V1.74). Returns the committed relationship projection (absent on remove), the new per-row version, and validation diagnostics.
 */
export interface WorldKbPatchRelationshipResponse {
  relationship?: NexusWorldKbRelationshipProjection;
  /**
   * New per-row version after the patch was persisted.
   */
  version: number;
  validation_summary: {
    errors: string[];
...
/**
 * Canonical projection of the row after the patch. Null/absent for remove.
 */
