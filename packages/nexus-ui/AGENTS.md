...
| `<ComputeResultNodeChrome>`, `<ComputeInspectorSections>` | V1.147 P2 — `2026-07-31-v1.147-p2-compute-on-timeline` T2 | 2026-08-01 | compute-on-Timeline primitive set (behavior spec §5). `ComputeResultNodeChrome` = Narrative node body (Cpu icon + kind pill + provenance chip + module/run meta; composes with the app `NodeChromeShell` — RF wrapper stays App-local per V1.124 extract pattern). `ComputeInspectorSections` = node inspector content (module + version + provenance + report/params digests + affected knowledge + Run id + Open Run affordance; provenance renders in every state). Pure presentational; caller-owned copy/callbacks. Studio fixture: `timeline-compute.tsx` (Components → Compute Timeline). |
