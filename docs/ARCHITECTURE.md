...
Nexus OSS is a **local-first creative writing runtime** plus the clients that talk to it: the Rust CLI and headless Connect runtime, open-source UIs, and shared wire contracts published as `@42ch/nexus-contracts`.

Three product surfaces, one producer:

| Surface | Role | Rule of thumb |
|---------|------|----------------|
| **`nexus42`** | **Producer** — direct-core authoring/operator CLI (`cli` cohort) | Owns local persistence through the Rust core; one-shot, no service lifecycle |
| **`apps/web`** | **Consumer** — Control Room + canvas SPA | Talks only through contracts + client interface |
| **`apps/desktop-electron`** | **Consumer** — Electron host around `web` | Native extras only; no second SPA or DTO set |

The headless **`nexus-runtime`** binary (Connect-only, `--no-default-features --features connect-host`) and the Electron-owned TS service are the two long-running hosts; neither is launched or statused by the CLI. The integrated `nexus42 daemon` group was retired in v1.193 P2.

Placement rule: runnable product surfaces → `apps/`; reusable Rust libraries → `crates/`; publishable npm libraries → `packages/`. App-owned nested Rust stays with the surface until it becomes a shared building block.
...
|------|---------|---------|
| **Local** | Rust core authority, orchestration, agent host, Creator / memory, narrative, World KB, User knowledge, moment assembly, web/desktop UI | `nexus42` CLI direct calls + `nexus-runtime` Connect; the Electron/TS service serves `/v1/daemon/*` for browser/desktop |
| **Cloud enhancement** | Platform HTTP, sync, registration, optional cloud context stage | CLI / `nexus-cloud-sync` — **never** through the local service HTTP |

**Hard isolation:** the local service host must not depend on cloud sync or cloud-domain crates, and must not expose cloud HTTP or sync proxies. Local UI is part of the local line: it talks only to the local API and must not assume cloud-only product surfaces.
...
3. **`nexus-knowledge`** owns World-scoped KnowledgeEntries / SourceAnchors (merged from former `nexus-kb` in V1.139) and User-scoped knowledge. Do not conflate either with the CLI **work file index** under work-scope `kb` routes.
...
schemas/  →  contracts (Rust + npm)
     ...
     apps/nexus42 (composition root)
                ↑
     apps/web  ←──  apps/desktop-electron (wraps web)
```
...
- **Local service host:** the Electron-owned TS service composes the native/TS authority and serves browser/desktop HTTP. The retired Rust host composition (`nexus-daemon-runtime`, v1.193 P2) is gone; `nexus-runtime` is the Connect-only headless cohort.
- **CLI binary:** direct-call composition root for authoring/operator leaves, ACP client paths and cloud CLI features (feature-gated; the Connect-only `nexus-runtime` bin is the separate headless product).
- **UI:** depends on contracts and a transport-agnostic client interface; screens must not call `fetch` / Electron IPC directly.
...
| Boundary | Decision |
|----------|----------|
| **Local API** | HTTP under `/v1/daemon/*`, served by the Electron/TS service (never the CLI). Default bind is loopback. Remote bind is opt-in and must stay fail-closed (API key + remote flag + TLS for non-loopback). |
| **NexusClient** | UI depends on the interface; browser vs Electron implementations swap at the edge. |
| **Desktop** | Same SPA as web; the Electron host adds service lifecycle and OS affordances (open / reveal / path guard). Path checks are authoritative in Rust against the active workspace root. |
| **ACP** | Nexus is an **ACP client**, not an agent/server. Agent host adapts to the user’s local agents; do not ship a competing agent runtime or public ACP server surface on the local service host. |

Forbidden on the local service host: sync proxies, platform registration passthrough, treating the host as an ACP Agent/Server.
...
- **`@42ch/nexus-ui`** holds publishable brand and promoted presentational primitives. Product screens, routing, and local-service wiring stay in `apps/web`.
- **Design Studio** (`apps/design-studio`) is a contributor gallery — not shipped to authors and not coupled to the local service host.
...
3. **Local service host ≠ ACP server** — client/host only.
4. **No local-service↔cloud Cargo or HTTP coupling.**
