...
| `2026-07-18-v1.123-timeline-first-ia-deepening` | P3 — Timeline-first IA deepening (global Timeline entry + Canvas shell Timeline prominence + list surfaces Timeline activity + cross-World/Work Timeline navigation) | Done | **Must** — "Timeline 一定要突出" deepens from route default to structural prominence; depends on P0 spec lock + P1/P2 surface availability. |
| `2026-07-18-v1.123-three-layer-zoom-experience` | P4 — Three-layer zoom experience differentiation (per-layer layout/density/visual language + zoom/pan + cross-layer navigation UX + per-layer empty-state copy) | Done | **Must** — "三层不一样的感受" is a caller-mandated product feature, not optional polish; depends on P0 + P1/P2 layers existing. |
