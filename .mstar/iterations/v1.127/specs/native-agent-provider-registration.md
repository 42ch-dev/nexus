...
This is the V1.127 down-payment on the **Harness pillar** (one of STRATEGY's Three Pillars): the daemon finally registers the native agent providers (Codex + Claude) that already exist in the codebase but have never been wired into the `HostManager`. The V1.116 residual `R-V1116P0QA-001` — "Discovery works (scan shows installed), but session creation fails" — has rolled forward **11 iterations** (V1.116 → V1.126) because every prior iteration deprioritized it for new feature work. V1.127 pays that debt.

**What T1 actually delivers (qc1 C-001 reframing):** The `AgentHostSubsystem` docstring (agent_host.rs:44-45) says "with providers already registered" — but `HostManager::new()` was empty and `register_provider` was never called. T1 wires both providers at boot, closing the architecture-coherence gap. This unblocks `create_session()` which previously failed with "provider not registered" (`manager.rs:257-258`). **Agent discovery was already working** via `POST /v1/daemon/agent-host/scan` (agent_host.rs:577 — independent PATH scan that does NOT consult `HostManager`); the user-visible fix is that **session creation against native providers now reaches the provider** instead of failing at the registration lookup.

**Scope-aware promise (per NG-13):** P1 ships the registration wiring — the `HostManager` invariant is met and `create_session()` reaches the provider's `launch()` path. If a provider's *internal* session-create handshake has a latent bug (e.g. CodexNativeProvider's `execute()` process-spawn path is broken), that becomes a V1.128+ plan candidate. The 11-iteration roll-forward ends because the wiring debt is paid, not because every provider is certified bug-free.
