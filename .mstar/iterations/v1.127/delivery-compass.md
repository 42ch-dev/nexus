...
- **AC-V1127-7** (P1 → S2): After daemon boot, `HostManager` has `codex-native` and `claude-native` registered (verified via `GET /v1/daemon/agent-host/providers` — T1 integration test). This closes the V1.116 residual `R-V1116P0QA-001` architecture-coherence gap: the `AgentHostSubsystem` docstring invariant ("with providers already registered", agent_host.rs:44-45) is finally met, and `create_session()` no longer fails with "provider not registered" for native providers. **Agent discovery** in the AgentPicker was already working via `POST /v1/daemon/agent-host/scan` (independent PATH scan at agent_host.rs:577 — does NOT consult `HostManager`); **session creation** end-to-end is the manual QA verification step. **Tester-visible:** with `codex` (or `claude`) installed locally, opening the AgentPicker shows the Codex (or Claude) agent in the list (discovery was already working); selecting it and creating a session reaches the provider (session-create was previously broken because the provider was not registered in `HostManager`). **Scope note:** the original V1.116 QA finding was "Discovery works (scan shows installed), but session creation fails" — T1 closes the session-create prerequisite (registration), not a discovery bug. If the provider's *internal* session-create handshake has a latent bug, that becomes a V1.128+ plan candidate per NG-13.
