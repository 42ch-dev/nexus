...
**Status**: locked (Phase 1 Review & Edit chain complete — @product-manager 5.1, @architect 5.2, @writing-specialist 5.3 returned; PM lock 5.4). `wire_contracts_changed: false` (polish iteration — no new DTOs, no schema changes). PR to `main` after Phase 3.
...
Inherited from V1.92 (see `v1.92-remote-access-hardening-delivery-compass-v1.md` §0.1). Key terms reused unchanged: **Daemon**, **Daemon API** (NOT "Local API" in new prose — R-V192HYG-001 sweeps residual stragglers), **Remote bind**, **Certificate fingerprint**, **TOFU**, **Trust anchor**, **Connection config**, **Setup screen**. No new V1.93 vocabulary met the CONCEPTS.md bar.
...
- **P-1 (Prepare) spans `knowledge/specs/daemon-runtime.md`** (§15.1 SAN-vs-bind-host normative section). No schema changes, no DESIGN.md changes, no codegen.
...
- `knowledge/specs/daemon-runtime.md` §15.1 carries the codified SAN-vs-bind-host policy as normative.
...
|---------|------|--------|-------|
| `2026-07-06-v1.93-prepare-spec-codification` | Prepare — daemon-runtime.md §15.1 SAN-vs-bind-host precision tightening (R-V192SPEC-001) | Todo | Architect-owned spec amendment; tightens existing §15.1 rule + adds §16.2 cross-references. Locks P0 implementation contract. |
| `2026-07-06-v1.93-tls-robustness` | TLS Robustness (P0 — SAN invalidation on re-bind + graceful-shutdown grace cap; R-V192P0-001/002) | Todo | `@fullstack-dev`, `crates/nexus-daemon-runtime/`. Parallel with P1. |
| `2026-07-06-v1.93-remote-connection-polish` | Remote-Connection Polish + Desktop Tests (P1 — storage validation + fingerprint error copy + reconnect hint + desktop tests; R-V192P1-001/002/003 + R-V192UX-001) | Todo | `@frontend-dev`, `apps/web/` + `apps/desktop/`. Parallel with P0. |
| `2026-07-06-v1.93-closure` | Closure — naming sweep (R-V192HYG-001) + QC tri-review + QA + compound + Profile B + PR to `main` | Todo | Naming sweep `@writing-specialist`; runs after P0 + P1 merge. |
...
- `knowledge/specs/daemon-runtime.md` §15.1 codifies the SAN generation rule (loopback always + bind host + wildcard skip) + the regenerate-on-mismatch behaviour as normative.
...
- **Current iteration (V1.93)** — *in progress*: the V1.92 remote-access surface's deferred polish lands coherently in one convergence iteration. TLS certs stay correct under rebinding (R-V192P0-001); graceful shutdown respects the grace cap (R-V192P0-002); the SAN-vs-bind-host policy is normative in the spec (R-V192SPEC-001); the remote-connection UX degrades gracefully and gives clearer errors (R-V192P1-001/002 + R-V192UX-001); desktop connection-config has test coverage (R-V192P1-003); legacy naming stragglers are swept (R-V192HYG-001). Tech-debt board cleared to zero-in-V1.93-scope. `wire_contracts_changed: false`.
- **Next iteration (V1.94) transition criteria**:
  - Trigger: V1.93 merged to `main`; deferred-features tracker updated; integration branch retired.
  - Selection input: PM reviews backlog against a now-polished remote-access product. Candidates:
    ...
    - **BL-09 standalone maturation dashboard** — the oldest deferred backlog item (since V1.79); SOUL data infrastructure is mature; a strong V1.94 feature candidate once V1.92/V1.93 polish is settled.
