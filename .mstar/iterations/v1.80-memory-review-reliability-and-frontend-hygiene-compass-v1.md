...
|---------|------|--------|-------|
| `2026-07-01-v1.80-memory-review-reliability` | P0 — Track A: Memory Review Reliability (REL-01 bounded/serialized/timeout + client drain) | Todo | Closes `R-V178P0-QC3-003`. Wire additive: `ReviewResponse` + optional `has_more`/`processed` → `@42ch/nexus-contracts` 0.14.0 → 0.15.0. Backend `crates/nexus-daemon-runtime` reuses the existing handler-local bounded `fetch_pending_reviews_page`; no `crates/nexus-local-db` change is planned unless reuse proves impossible. Web mutation: `apps/web/src/api/queries.ts::useReviewMemory`. |
| `2026-07-01-v1.80-frontend-hygiene-residuals` | P1 — Track B: Frontend Hygiene (4 V1.79-QC residuals) | Todo | Closes `R-V179P0-QC1-001/002` + `R-V179P1-QC1-001/002`. `apps/web` reading/soul/memory-page components only. No wire change. |
| `2026-07-01-v1.80-closure` | P-last — Closure (compound + compaction + PR + tracker/STRATEGY + residual close-out) | Todo | Profile B compaction (3 plans). Update deferred tracker (REL-01 + 4 residuals → shipped). STRATEGY decision-log entry. |
...
- **Current iteration (V1.80)**: Reliability investment iteration. Pays down the accumulated `R-V178P0-QC3-003` (REL-01) reliability debt on the memory-review pipeline and four low `V1.79-QC` frontend hygiene residuals. No new product surface; the goal is a more robust foundation for the next feature iteration. First "stabilize before extending" iteration after four consecutive feature iterations (V1.76–V1.79).
