...
**What to improve**
- The implementer twice wrote the SDD report to the wrong location (`.mstar/plans/reports/` instead of the gitignored SDD dir) and committed it once — the brief now states the path more firmly, but a `sdd-workspace`-relative convention or a pre-commit hook could prevent recurrence.
