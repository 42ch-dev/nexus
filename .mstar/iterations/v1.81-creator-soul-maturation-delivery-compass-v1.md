...
|---------|------|--------|-------|
| `2026-07-01-v1.81-prepare-spec-and-contracts` | P-1 — Prepare (specs + wire contracts + codegen + DESIGN.md) | Todo | Specs (web-ui SOUL section, creator-memory-soul-lifecycle) + wire DTO contract authoring + **codegen + 0.15.0→0.16.0 + drift registration** (contract landing moved from P0 to P-1 so P0‖P1 parallelize against frozen generated types) + DESIGN.md tokens. Locks contracts. QC skipped (contracts/codegen — no runtime behavior). |
| `2026-07-01-v1.81-creator-soul-narrative-and-world-foundation` | P0 — Backend: Creator-SOUL Narrative + per-World foundation | Todo | Schema migration (`memory_fragments.world_id`) + threading at promotion seam (3 `create_fragment` sites) + DAO world filter + additive DTO codegen + soul-narrative synthesis (trait seam + agent invocation + endpoint + persistence + stale-invalidation). Owner `@fullstack-dev`. |
| `2026-07-01-v1.81-soul-surface-deepening` | P1 — Frontend: SOUL Surface Deepening (narrative card + world selector + growth-curve + auto-refresh) | Todo | Narrative card consuming the new endpoint + world-selector drilling keyword/drift/growth + growth-curve viz component + auto-refresh (poll + post-review invalidation). Owner `@frontend-dev`. Builds against P-1 frozen contracts. |
| `2026-07-01-v1.81-closure` | P-last — Closure (compound + 2 V1.80 low doc residuals + compaction + PR + trackers/STRATEGY) | Todo | Compound round; close 2 V1.80-P0 low doc residuals (R-V180P0-QC1-001 mutex-map lifecycle doc + R-V180P0-QC2-001 best-effort delete doc) as companion; Profile B compaction; deferred-tracker (BL-10 shipped, BL-12 shipped minus per-world-narrative) + STRATEGY decision-log update. |
...
- **Current iteration (V1.81)**: Returns to the feature cadence after V1.80
  stabilization; deepens the V1.79 reflection axis. Headline = Creator-SOUL
  Narrative (on-demand + stale-invalidation, world-agnostic).
  Companions = per-World projection (data foundation + UI selector), independent
  growth-curve (BL-10), and auto-refresh. Additive wire DTO (`memory-fragment-info`
  + `world_id`; new soul-narrative schemas) → `@42ch/nexus-contracts` 0.15.0 →
  0.16.0. Ships the user-locked "Creator SOUL (whole) vs World projection
