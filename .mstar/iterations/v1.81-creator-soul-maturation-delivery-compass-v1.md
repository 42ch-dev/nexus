...
|---------|------|--------|-------|
| `2026-07-01-v1.81-prepare-spec-and-contracts` | P-1 — Prepare (specs + wire contracts + DESIGN.md) | Todo | Specs (web-ui SOUL section, creator-memory-soul-lifecycle) + wire DTO contracts (`memory-fragment-info` + `world_id`; new soul-narrative request/response) + DESIGN.md tokens. Locks contracts so P0‖P1 parallelize against frozen shapes. QC skipped (docs/contracts/token-stubs only). |
