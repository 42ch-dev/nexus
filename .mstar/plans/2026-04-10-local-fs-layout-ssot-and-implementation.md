...
**Architecture:** Introduce a small **path resolver** that maps `(creator_id, workspace_slug)` → DB path; persist **active `creator_id`** and **per-creator active `workspace_slug`** (fallback **`default`**); implement **`nexus42 creator workspace`** subcommands per `.mstar/knowledge/specs/cli-spec.md` §6.2C; keep **nexus-local-db** as SQLite owner. (No legacy flat `state.db` or migration command — product pre-release.)
...
In this plan, **`{v1-spec/…}`** denotes a path **relative to** `specs_root.v1-spec` from **`.mstar/local-paths.json`** (copy from `.mstar/local-paths.json.example` if missing).

| Variable | Resolves to (relative) |
|----------|-------------------------|
| `.mstar/knowledge/specs/cli-spec.md` | CLI + directory layout SSOT |
| `.mstar/knowledge/specs/local-db-schema.md` | Local SQLite filename + module boundaries |
...
**OSS companion (reachable without platform checkout):** `.mstar/archived/knowledge/local-fs-layout-creator-workspace.md` (non-normative recap + pointer)
...
| Area | Responsibility |
|------|------------------|
| `.mstar/archived/knowledge/local-fs-layout-creator-workspace.md` | OSS companion — pointer to ADR-014 |
| `{v1-spec/adr/adr-014-local-fs-creator-workspace-layout-v1.md}` | Normative ADR |
| `.mstar/knowledge/specs/cli-spec.md` | User-facing command SSOT (`creator use` + `creator workspace *`) |
| `.mstar/knowledge/specs/local-db-schema.md` | Canonical **on-disk** path for `state.db` |
...
- Read: `.mstar/archived/knowledge/local-fs-layout-creator-workspace.md` (OSS companion)
- Read: `.mstar/knowledge/specs/cli-spec.md` §6.2B–§6.3, §13.2
- Read: `.mstar/knowledge/specs/local-db-schema.md` §0
- Read: `{v1-spec/domain/data-model-v1.md}` §5.14

- [ ] **Step 1:** Open `.mstar/local-paths.json` (or create from `.mstar/local-paths.json.example`) and confirm `specs_root.v1-spec` resolves.
...
- [x] **Step 1:** Change `init` so **creative root** default matches configurable template **documented in ADR** (e.g. `Documents/nexus/<creator_slug>/<workspace_slug>/`), still creating `Stories/` and `References/` per `.mstar/knowledge/specs/cli-spec.md` §13.1.
...
- Cross-check: `.mstar/knowledge/specs/cli-spec.md` §6.2B–§6.2C
...
3. **Type consistency:** **`workspace_slug`** (path segment) vs Wire **`workspace_id`** (optional in `meta`) vs `creator_id` match cli-spec / contracts; `state.db` path matches `.mstar/knowledge/specs/local-db-schema.md`.
...
**Plan complete** (nexus repo): `.mstar/plans/2026-04-10-local-fs-layout-ssot-and-implementation.md`
