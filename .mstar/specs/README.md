...
| [spoke-adapter-architecture.md](spoke-adapter-architecture.md) | Master | **Normative (v0.3 — V1.142 SPOKE 0.4.1 pin + production adapter home + first orchestrator cutover)** — spoke-adapter boundary, `extensions.nexus` namespace contract, daemon-api envelope strategy, nexus-contracts package boundary, drift adaptation, HTTP route stability, spoke-operations call-boundary invariant, dual-surface adapter API (pure delegates + injection orchestration), production `NexusBaselineAdapter` home (`nexus-local-db`), production-vs-stub port matrix |
