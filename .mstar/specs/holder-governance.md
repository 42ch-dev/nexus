...
> **Status:** Shipped (v1.191 P1) — implemented and exercised on the v1.191 plan branch (`feat/v1.191-p1-spoke-0-13-adoption`); integration into `iteration/v1.191`/`main` (the iteration PR) remains PM-owned and no merge artifact exists yet. Current SPOKE pins are the pinned upstream lockstep release recorded in the workspace manifests (D17). Exercised evidence: `.mstar/sdd/2026-09-18-v1.191-p1-spoke-0-13-adoption/task-15-report.md` §2/§AC4–AC10.
...
**Collected evidence (v1.191 P1).** Every row above is discharged by the plan's task gates: `git diff --check` clean over the declared cutover paths; no reachable production path uses an unscoped KE reader (dead, uncalled unscoped storage APIs remain and are tracked as residual R7 — see the T15 report §9 G2); the acceptance matrix, per-row gate citations, promoted docs and residual owners are recorded in `.mstar/sdd/2026-09-18-v1.191-p1-spoke-0-13-adoption/task-15-report.md` (§AC4–AC10 mapping and §§10-reconciliation). Residual items that are *not* discharged by this contract are listed there with owners; nothing in this document claims a behaviour that was not exercised.
