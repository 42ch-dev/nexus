...
New local-db `holders.rs` owns transaction-taking `ensure_creator_holder_in_tx`, `ensure_character_holder_in_tx`, and read-only `resolve_holder`. Existing Creator materialization (`creators::ensure_creator_row` and every other creator-creation caller) converges on that one transactional implementation — the second caller used to be the daemon `handlers/creators.rs` helper, which was deleted with the host in v1.193 P2. Global registration need not atomically write every workspace; before a workspace identity becomes usable its local subject and registry row must commit together. Character creation adds its registry row in the existing create transaction. Backfill includes archived subjects.
...
Apply SQL predicates before keyset cursor, LIMIT, count, ranking/snippets, join expansion and truncation checks. Search filters eligible row ids before scoring/returning snippets. A hidden read-by-id is indistinguishable from missing. Relationship endpoints, findings and compute reads cannot reveal hidden row ids or payloads; filter/reject the containing operation before it produces output. Do not cap all rows then filter in Rust. CreatorManagement uses explicit owned-container checks and known-governance admission, not the ActorView wire helper. **Keyset order (settled — v1.194 P2-T5):** the SQL keyset expression and the Rust merge key MUST produce the same **truncated** sub-millisecond key for the same stored timestamp (truncation of the stored fraction, never SQLite `strftime('%f')` rounding, which disagreed by one millisecond for fractions at/above half a millisecond and could drop an eligible row from a page walk). The comparison is the row value `(created_key, entry_id) > (cursor_key, cursor_id)` with the entry id as the stable tie-break; the eligibility/visibility predicates stay in the `WHERE` before cursor and `LIMIT`, so one total order governs SQL eligibility/cursor/limit and the Rust merge. Cursor token bytes, stored timestamp bytes and the placeholder/bind contract are unchanged, and a walk over all eligible rows returns each exactly once with a bounded terminating cursor.
...
1. Quiesce CLI/core/Connect writers and in-flight Actor activities; acquire existing exclusive database/writer admission. Back up the complete database before DDL. No old process remains attached during migration.
...
| `apps/nexus-service/src/{actors,world-kb,knowledge}.ts` + the `nexus-core-node` actor/domain projections | Existing routes delegate to the core policy; reject raw old keys before deserialization can discard them. The original owner was the deleted `nexus-daemon-runtime/src/api/handlers/{actor_knowledge,characters,creators,world_kb,world_kb_guards,world_kb_pack}.rs` (v1.193 P2) |
...
| `apps/nexus42/src/commands/creator/{character,kb}.rs`, `creator/world/kb/pack.rs` | Existing add/list/view/show/edit/remove, `--audience shared\|author-only\|character-private` plus `--audience-character <id>`; edit requires expected revision. Keep the canonical `creator world kb` family (the historical `creator kb --scope world` alias was deleted in v1.193 P2), add no new command family |
| `nexus-core/src/world_pack.rs::{run_world_pack_import,import_pack,export_pack}` (the pack-import translation now lives here; the former daemon `pack_import.rs` module was deleted in v1.193 P2) | Core owns mapping/quarantine/conflict policy and scoped export; CLI direct export must converge on this admitted path rather than loading a raw store |
...
| `apps/nexus42/src/connect/{allowlist,invoke}.rs`, core Connect accept/responder, and the Connect host composition (`nexus-runtime`) | §9 matrix and authenticated stored grants |
...
Evidence must cover two Creators, two Characters, two Worlds, multiple bindings; shared/Creator-private/Character-private/unknown/malformed rows; retained archived reads; rename/restore id stability; old-bool migration equivalence; conflicting migration rollback; spoofed holder/viewpoint and foreign-import id collision; hidden rows before page boundaries/search scoring; governance CAS versus active session/context reuse; WorldSheet reverse-reference edits; every Connect served family; and both real local extraction callers with cancellation/no-write and relationship preservation. Use existing focused crate tests and CLI/service smoke paths, not a broad local suite or a helper-only claim.

**Collected evidence (v1.191 P1).** Every row above is discharged by the plan's task gates: `git diff --check` clean over the declared cutover paths; no reachable production path uses an unscoped KE reader (dead, uncalled unscoped storage APIs remain and are tracked as residual R7 in the plan's evidence); the acceptance matrix, per-row gate citations, promoted docs and residual owners are recorded in the plan-level v1.191 P1 task reports (harness-process paths, deliberately not linked from a tracked doc). Residual items that are *not* discharged by this contract are carried there with owners; nothing in this document claims a behaviour that was not exercised.
