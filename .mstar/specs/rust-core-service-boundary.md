...
- The single authorized live request stays open and undispatched: it stops at `credentials_unavailable` with zero admissions, because credential usability cannot be established without inspecting the value, which the policy forbids. A transport or authentication failure **after** an admission would consume the authorization; there is no retry without a new explicit user grant. Retired daemon commands stay retired.
...
- Unauthorized paid or live model requests. Historical P5 public first-run stays unshipped on `main`; the v1.195 P3 deterministic example is loopback-only and is not live qualification. v1.195 may execute only the one short request named by that iteration's Decision D5, after deterministic prerequisites and a one-request guard; credential unavailability leaves that request undispatched, an admitted live failure consumes the authorization, and neither case reopens general live spend.
