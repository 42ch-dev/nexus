...
- Patch schemas are closed (`additionalProperties:false`). Omission keeps a field; explicit null clears only nullable fields. Decode the original member presence before mapping a generated optional value: generated `Option<T>` alone must not collapse missing and null. Do not introduce a second handwritten wire DTO. An empty patch is `400 invalid_input`; a supplied identical value is a CAS-checked no-op (no revision/timestamp change).
- For owned targets: active-write gate precedes CAS; CAS precedes dependent-resource validation and uniqueness. Same-state lifecycle requests first check CAS, then return the unchanged detail. Invalid wire structure is `400 invalid_input`; database faults remain `500 internal` through the canonical `NexusApiError` envelope.
