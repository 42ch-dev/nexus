...
**v1.191 holder amendment (shipped):** [holder-governance.md](holder-governance.md) is the technical authority for stable Creator/Character holder KEs, native disclosure, the complete `creator_only` replacement, author controls and trusted management/view policies. The holder contract is exercised on the v1.191 plan branch; §5.4, §11.4 and §11.5 below carry the shipped holder-era condition, and the pre-cutover bool descriptions remain only as the shipped-as-of-v1.185 history they describe.
...
### 5.4 Holder policy (shipped v1.191)

`KnowledgeOwnerRef` still selects the three narrative containers above. Holder governance is a separate conjunct: shared rows have no disclosure; `owner-private` rows are visible only to the exact admitted holder on Character/Connect views; unknown disclosure is excluded. Creator management review remains authorized over all owned known-private entries, including Character-private material. It is explicitly chosen by core admission and is never a missing-viewpoint bypass.

One stable service-managed holder KE belongs to each Creator and Character identity, independent of WorldSheet and binding. Existing knowledge commands carry shared/author-only/Character-private audience controls with revision CAS. `creator_only` World rows migrated to the stored controlling Creator holder, preserving prior Character invisibility, and the bool is removed from every runtime path. Registry, wire/native names, migration, trusted readers, session invalidation and public schema paths are defined once in [holder-governance.md](holder-governance.md); no second authorization taxonomy is defined here.
...
Within `BEGIN IMMEDIATE`, require the owned active Character, exact active binding and owned active World, compare binding revision, then validate any new link against `kb_key_blocks`: `owner_kind='world'`, matching `world_id`, `block_type='character'`, live status not in `deleted|merged|deprecated`. A sheet carrying disclosure (`owner-private` or unknown vocabulary — the retired `creator_only` case included) rejects with `invalid_world_sheet`, so its identity link cannot leak private lore through Character reads. Wrong type/World/owner, missing and non-live all use the same `invalid_world_sheet` conflict. Apply this validation to existing create/add as well as update. Clearing never edits or deletes the old sheet.

**Shipped holder condition (v1.191):** the sheet guard is “disclosure absent”, applied in both directions — linking a sheet, and editing the governance/status of an already-linked KE. The retired `creator_only` predicate is not retained as a second predicate.
...
**Shipped holder condition (v1.191):** the existing content endpoint also accepts the closed `audience` patch under the same `expected_revision`, preserving governance on omission and bumping the revision once for a material combined update. Narrative owner/block/status/provenance remain non-transferable. Raw holder ids and the retired `creator_only` key are not author controls (the key is refused by presence, including `false`). The exact contract is [holder-governance.md §3](holder-governance.md#3-native-governance-and-atomic-authoring).
