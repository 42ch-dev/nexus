...
## 9. Workspace Session OCC (V1.56; V1.188 recoverable content commit)

### 9.1 One authority

HTTP workspace routes and orchestration `workspace.open` / `workspace.commit`
use the same daemon-owned workspace session manager and local-DB authority.
Contracts own the shared camelCase DTOs; orchestration consumes an injected
async `WorkspaceExecutor`. An absent executor is an explicit failure, never
a synthetic path, consumed-only success or fabricated revision.

`open({path})` resolves a safe scope, scans regular-file SHA-256 pre-images,
persists the session and returns
`{sessionId,snapshot:{workspaceRoot,path,existed,fileHashes}}`.
Snapshot and change paths are relative to that opened scope.

### 9.2 Target-content commit

`commit({sessionId,changes:[{path,op,expectedHash,contentBase64}]})` carries
target bytes for create/modify and opened pre-image hashes for modify/delete.
Create requires absence; delete forbids content. Only regular files with
existing parents are supported. Validate the whole manifest before staging:
at most 128 changes, 1 MiB decoded per file and 8 MiB total; reject malformed,
duplicate, ancestor-overlapping, unsafe or symlink paths and corrupt snapshots.

The transition is: validate pre-images → exclusively stage sibling target
files and original backups with fsync → persist intent and stable revision →
apply atomic per-file replacements plus directory fsync → atomically record
committed and consume the session → return `{revision,committed:true}`.
The same committed request after a lost response returns the same revision
without repeating effects; another request on that session conflicts.

### 9.3 Retained recovery

Ordinary apply failure durably selects rollback and restores pre-images before
reporting rolled back. Before admitting new writes, restart recovery finishes
applying or undoes rolling-back intents idempotently using pre/post hashes.
The recovery owner survives client/workflow cancellation. Open-session TTL
gates a new intent only; it does not discard admitted recovery.

Validation failure before intent leaves the open session usable. A rolled-back
intent closes it to another attempt; restart between open and commit preserves
a usable open session. A third-state file, changed ancestor, or missing/corrupt
recovery material becomes actionable `recovery_conflict`: retain evidence and
backups, block writes and never overwrite unrelated bytes or publish committed.

### 9.4 Visibility and external-writer boundary

This is recoverable multi-file commit, **not multi-file filesystem atomic
visibility**: external readers can observe a short inter-file transition.
Nexus locks/CAS do not prevent unrelated writers racing the final check/rename.
Guaranteed completion/rollback requires those writers' quiescence. Exceptional
recovery conflict can leave transitioned bytes until repaired; it must not
claim “nothing changed.” Clients reopen after pre-image conflict rather than
receiving an automatic retry. This file-content domain remains separate from
DB row-version OCC (§7).
