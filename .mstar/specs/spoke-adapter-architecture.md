...
| **N-C3** (V1.155, delivered) | `list_peer_host_capability_manifests` production / multi-host — `peer_hosts` table (workspace DB, migration `20260808120000_peer_hosts.sql`); recording at the outbound `connect()` return (manifest-backed upsert, `last_seen` RFC 3339 UTC, lock #1 fallback: inbound-only peers not recorded — spoke-connect inbound-manifest API change, out of nexus scope); empty → `Ok(vec![])` (stub contract preserved); corrupt stored row → `InternalError`; honesty: only observed peers, never fabricated; operator read = `connect peers list` via `NexusAdapter::list_observed_peer_hosts` (manifest + `last_seen`). Zero adapter stubs remain. | DF-72; closes `R-V1142P1-002` |
