...
| **N-C2** (read half, delivered) | `check` / `assemble` over Connect — served-op set = `upsert` / `promote` / `relate` / `check` / `assemble` (`SERVED_OPS` ⇔ manifest `extensions.nexus.served_ops`, honesty machine-checked both directions over the enlarged set, literal `"reasoning-complete"` asserted absent); roles = `["data-store", "checker", "assembler"]` (open-string vocabulary; `checker` / `assembler` back the served read ops); read ops ride the same fail-closed world/op scoping as writes + the bounded async bridge (architect-locked `BridgeLimits`: **8** concurrent invokes per process, **30,000 ms** per-invoke deadline, **500** logical collection entries or **2 MiB** serialized request bytes — whichever is reached first; envelope codes `invoke_busy` / `invoke_deadline_exceeded` / `payload_too_large`); literal `"reasoning-complete"` still **not advertised** — the semantic milestone lands with `compute` (P2) | DF-72 (read half); `compute` = P2 |
