...
> **Holder amendment (v1.191 P1 — shipped):** [holder-governance.md](holder-governance.md) owns the complete Actor-holder registry, native disclosure, trusted read policies, cutover and production extraction contract. The package pins moved together to the iteration's pinned upstream lockstep release (version SSOT: the manifest pins; compass D17 records the adoption decision). `creator_only` was removed only by that complete migration, not by a wire-only version bump.
...
| ~~`creator_only`~~ | ~~boolean~~ | **Retired (v1.191 P1)**: the key is refused on raw input (`legacy_creator_only_unsupported`, `false` included) and removed by every typed write | ~~`KnowledgeEntryRecord.creator_only`~~ |
...
Exactly one narrative owner key is emitted; Character/binding entries never fabricate `world_id`. **Shipped (v1.191 P1):** native `holder_entry_id` maps to SPOKE core `owner`, and native `disclosure` maps to core `disclosure`; neither is an extension. The seven nexus keys keep their container/provenance meaning. The retired `creator_only` key is explicitly rejected on raw input, never preserved as an unknown key.
...
The adapter merges known columns with unknown extras on read and writes both consistently. Unknown module/extension preservation remains mandatory. The shipped governance adds authoritative `holder_entry_id`/`disclosure` columns and a separate service-managed holder registry; storage constraints, pre-limit filtering, the offline `creator_only` removal, writer fences and rollback are owned solely by [holder-governance.md §§2–6](holder-governance.md#2-registry-identity-stability-and-lifecycle). A clean wipe is not an acceptable migration of authored data.

### 2.4 Ownership and extraction boundaries (shipped v1.191 P1)
...
1. Verify the three crate pins (`spoke-schemas`, `spoke-operations`, `spoke-connect`) all match the pinned lockstep release in `Cargo.toml`.
2. Verify the two npm pins (`@42ch/spoke-schemas`, `@42ch/spoke-operations`) both match the pinned lockstep release in `package.json`.
...
DF-74 delivered V1.149; DF-75 delivered V1.150; DF-76 delivered V1.151 (assembly inspector).

#### Slot vocabulary + Moment Directive + generation-stage gates (V1.150 / DF-75 — shipped)

V1.150 (DF-75, plans P0+P1+P2) adds **product-local** assembly shaping on top of the V1.149 activation engine. **Consumer-only discipline holds:** no new spoke wire, no `spoke-operations` changes, no new `schemas/` DTOs (`wire_contracts_changed: false`). Product behavior SSOT: iteration draft `fl-l-w5-prompt-control-plane.md` (promoted to tracked status here at P2 close).
...
DF-74 delivered V1.149; **DF-75 delivered V1.150**; DF-76 delivered V1.151.
...
spoke-only fields (`fork_id`, `parent_fork_id`, `timeline_scale`, `source_anchor`, `computable_logs`) are lossily filled (`None`/empty) — **durable roadmap:** DR-44 (spoke fork-model participation).

**Beat-assist helper adoption:** `order_timeline_events_by_ids` is the primary production adoption target (V1.143 P0 T2). The helper operates on `timeline_event_id` alone and requires no peripheral infrastructure. `order_timeline_events_by_precedes` — **durable roadmap:** DR-45 (`order_timeline_events_by_precedes` adapter adoption).
...
Normative architectural surface for the first FL-R Connect Host slice. Product behavior detail (integrator persona, dogfood story, non-goals) lives in the iteration draft `fl-r-connect-host-foundation.md` until promoted; this section locks durable topology and honesty rules for implementers and QC.
...
- Capability-token (V1.155 P1, **production**): issuance + operator config + enforcement, all opt-in and fail-closed — an absent `~/.nexus42/connect/config.json` keeps the pre-V1.155 defaults (`trusted_issuers` empty, `capability_token_provider = None`, `require_capability_token = false`), so existing hosts behave exactly as before.
  - **Issuance — `nexus42 connect token issue`:** flags `--sub <peer-id>` / `--aud <peer-id>` / `--capabilities <c1,c2>` / `--exp <unix-seconds>` (all required), optional `--iss` (MUST equal the issuer-derived peer id — spoke normative rule "issuer key MUST derive claims.iss"; fail-fast otherwise). Output: the signed wire proof `{v, claims, sig}` (JCS + Ed25519, base64url sig) as compact JSON on stdout, exit 0; missing/ambiguous args → usage error exit non-zero; no secrets echoed. Issuer key lifecycle: `~/.nexus42/connect/issuer.key` (Ed25519, create-once 0600, TOCTOU-safe, never overwrites; distinct file from `identity.key` — node identity vs token-issuer are different trust roles); issuer peer id derives from the key (`derive_peer_id_from_ed25519_pubkey`); `iat` stamped at issue, `exp` must be beyond now + 60 s skew (spoke fail-fast parity). CLI issue is the ONLY issuance path — no revocation list / refresh / issuance endpoint (spoke offline-validation non-goal preserved).
  - **Operator config — `~/.nexus42/connect/config.json`** (new file; `allowlist.json` stays the peer/scope file): `{ trusted_issuers: string[], require_capability_token: bool, capability_token_provider: { enabled: bool, issuer_key_path?: string } }`, serde `deny_unknown_fields` on both levels. Absent file ⇒ defaults (empty / false / None — existing behavior unchanged); malformed / unknown-field / unreadable ⇒ fail-closed boot error carrying the path (no silent defaults); `require_capability_token = true` with empty `trusted_issuers` ⇒ boot error (the host would challenge every session but accept no proof). `capability_token_provider.enabled` ⇒ `build_config` loads the issuer key at boot and constructs the mint-on-demand provider closure (spoke `issue_capability_token`; `iss` = issuer-derived peer id, `sub` = this node's peer id, `aud` = challenger, capabilities = the host's advertised manifest capabilities — a token can never grant more than the host advertises; TTL 300 s; key material captured at boot, zero I/O on the network event loop). `issuer_key_path` optional — default `~/.nexus42/connect/issuer.key`; relative paths anchor at `~/.nexus42/connect/`.
  - **Enforcement (spoke-side, fail-closed):** when `require_capability_token = true` AND `trusted_issuers` non-empty, spoke-connect's `evaluate_invoke_token_gate` (node.rs) enforces the token-required assertion BEFORE the nexus invoke handler runs — a session that has not completed the token challenge is rejected with the `auth_failed` wire envelope, zero side effects (the nexus handler never sees the invoke). The op-dispatch capability gate (`required_capability` core table + `negotiated_allowed` + `token_authorizes_op`, spoke dispatch.rs) covers the 6 served ops (all core-table: upsert/promote/relate/check/assemble → `spoke-baseline`, compute → `l2-computable`); `op_capability_requirements` stays empty (no parallel invention). Nexus does NOT re-implement the assertion — it only wires `build_config` (V1.155 P1 T2).
  - **Tenant isolation (composition invariant):** effective grants = nexus `PeerScope` scope ∧ spoke-side (negotiated ∩ token). The nexus `PeerScope` gate (invoke.rs, N-C1) enforces `world_scope` / `op_scope` / `module_scope` independently of the token — a token granting `l2-computable` to a peer whose `op_scope` lacks `compute` is denied nexus-side (`op denied: …` message family) while the spoke gate passes, so the token CANNOT widen the allowlist scope. No new nexus gate code (V1.155 P1 T3 verifies the composed invariant with interop tests). No change to `SERVED_OPS`; no change to allowlist semantics.
...
- V1.148 also lands **`POST /v1/daemon/check`** (tier2, `is_world_owned`, adapter `orchestrate_check`, no KB auto-apply beyond FindingPort persistence). That route is **creator Daemon HTTP only** — it does not make Connect `check` available. .
...
| **Capability-token production** (V1.155 P1, delivered) | Issuance + operator config + enforcement + tenant isolation, all opt-in and fail-closed — `nexus42 connect token issue` CLI (issuer.key Ed25519 create-once 0600, `claims.iss` = issuer-derived peer id); `~/.nexus42/connect/config.json` operator surface (`trusted_issuers` / `require_capability_token` / `capability_token_provider{enabled, issuer_key_path}`, deny-unknown-fields; absent ⇒ pre-V1.155 defaults; malformed / require-without-issuers ⇒ fail-closed boot error); enforcement spoke-side (`evaluate_invoke_token_gate` ⇒ `auth_failed` before the nexus handler, zero side effects; op-dispatch capability ∩ token); nexus `PeerScope` gate unchanged — token can never widen allowlist scope (effective grants = PeerScope ∧ negotiated ∩ token); defaults stay `require=false` / empty issuers / no provider | PD-09; closes `R-V1148P3-001` |
...
`pack-io-product-behavior.md` (V1.146 P3 shipped
