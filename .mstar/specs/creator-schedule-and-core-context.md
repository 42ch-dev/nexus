...
Per [orchestration-engine.md](orchestration-engine.md) §6.2, one creator has one ACP worker subprocess in V1.4 MVP, and that worker holds **one** ACP session. This puts a hard cap on **parallel ACP-calling** Schedules for the same creator:
...
Per [schemas-external-consumer-boundary.md](../knowledge/schemas-external-consumer-boundary.md) §2, these request/response types are **local** (platform never observes them). Request/response Rust types live in `crates/nexus-contracts/src/local/schedule/http.rs` — hand-written, no JSON Schema file.
