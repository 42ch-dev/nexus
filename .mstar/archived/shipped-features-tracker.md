...
**Created**: 2026-05-30  
**Last updated**: 2026-06-22 (V1.56 closeout: 7 plans all Done — P-1 prepare + P0 DF-31 full + DF-42 Local API redesign + P1 DF-29 registry.refresh + P2 DF-56 independent slice (3 sub-items: arbitrary stage conditional + expression grammar + merge points) + P3 DF-56 dependent slice (2 sub-items: registry + workspace branch inputs) + P-mid 3-wave QC rhythm + P-last closeout incl. R-V155P2-F002 fix-wave; DF-29/31/42/56 closed; R-V155P2-F002 closed; 36 V1.57+ carry-forwards registered; wire contracts changed)
...
| ~~DF-56~~ | Conditional routing / branching engine (full roadmap) | V1.56 (P2+P3) | V1.42 P2 shipped minimal slice (llm_judge GO/NOGO); V1.52 T-B P0/P1 shipped N-way + merge semantics; V1.56 closes remaining 5 sub-items via 2 plans: **P2 independent slice** (任意阶段条件 next + 表达式/规则路由 Form A+B + multi-branch + merge points with `wait_for_all`/`first_completed`/`any` strategies + expression parser with `MAX_EXPR_DEPTH=32` + JSON equality semantics) + **P3 dependent slice** (registry.refresh + workspace branch inputs in expression routing). Total: 821 lib tests + 11 converge e2e + 20 P3 integration tests. P2 fix-waves: H-001 converge runtime gap (closed); C-NEW-001 dedup bug caught by qc3 revalidation (closed). Spec extend `preset-conditional-routing.md` §3.3+§3.4; PM amend `entity-scope-model.md` §5.5.8. |
...
**Note**: V1.56 P-last added V1.56 delivery snapshot directly. PR to `main` pending user push authorization. Profile B compaction (7 plans archived + `plans-done.json` index updated + `metadata.latest_ship` updated). V1.55→V1.56 carry-forward closure complete (0 open).
