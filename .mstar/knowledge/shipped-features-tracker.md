...
**Purpose**: Long-term **append-only** archive for closed deferred-feature tracker rows (shipped, cancelled, superseded) and per-version delivery snapshots. The **active** open/backlog tracker lives in [deferred-features-cross-version-tracker.md](deferred-features-cross-version-tracker.md).  
...
|----|---------|------------|-------|
| Mental-layer pipeline close-out (V1.165) | World-scoped check findings persistence + observation authoring | V1.165 | P1: `world_findings` table (DB v19, spoke vocabulary verbatim, FK→narrative_worlds) + FindingPort `extensions.nexus` scope discriminator (work path byte-identical; world path new; both/neither reject) + checker stamp fix (world_id, the 500 root cause) + `GET /v1/daemon/worlds/:world_id/findings` (ownership-gated, SQL LIMIT 501-probe/500-cap + truncated flag — Bugbot fix 226a7c5a); world check E2E 500→200 with dramatic_irony_asymmetry persisted + read back; legacy work findings untouched. P2: `modules` additive on WorldKbEntityPatch (first-level key upsert merge; omit=preserve; {}=no-op; ModuleMap shape guard + fail-closed promote guard) → mental/belief/observation authoring via product patch path + CLI `creator world event-add --observer/--access` (checker-consumed log carrier); Canvas E2E via product write path (PATCH→graph→inspector Observers line). Dual carriers stay dual (PD-9). Spec: [v1.165-pipeline-closeout-locks.md](../iterations/v1.165/specs/v1.165-pipeline-closeout-locks.md). Plans: `2026-08-14-v1.165-p{1,2}-*`. |
| L5-mind mental layer (V1.164) | Who-knows-what dimension on the World KB spine | V1.164 | spoke 0.9.2→0.10.0 lockstep (Rust×3+npm×2+drift gate) + `TimelineEvent.modules` bidirectional passthrough + `narrative_timeline_events.modules_json` (v17); `modules.mental` (nine-field) / `modules.belief` (handbook propositions) on KnowledgeEntry + `mind_states` derivative table (v18, validate gate at adapter boundary); orchestrateCheck checker pair `stale_belief_drift` (warning) / `dramatic_irony_asymmetry` (info); Canvas read-only surfacing (`modules` additive on WorldKbEntityProjection + TimelineEventInfo, Studio fixture, character Mental State section, event Observers line). Engines (belief revision / ToM) stay product-side future. Open: R-V1164P2QC-001 (high, world-scoped findings FK 500) + R-V1164P3QC-001 (medium, observation authoring path). Spec: [v1.164-mental-layer-product-locks.md](../iterations/v1.164/specs/v1.164-mental-layer-product-locks.md). Plans: `2026-08-14-v1.164-p{1,2,3}-*`. |
...
| ~~DF-V1122-FORK-UI~~ | Fork creation + read-only lineage chrome (create half) | V1.162 | Fork spine completion — local authoring fork (`nexus.fork.create` V1.60 P0 + V1.162 daemon `POST /v1/daemon/worlds/:world_id/forks` thin-delegate + World Timeline fork-point create + PD-6 post-create landing into forked branch) + read-only lineage chrome (fork-badge + parent + fork-point + one-hop parent, marker-derived carrier B). Carrier B: lineage on `fork_created` canon marker `extensions_nexus_json.fork_lineage`. **fork-merge remains deferred (BL-01).** Spec: [entity-scope-model.md](../specs/entity-scope-model.md) §6.6.1-6.6.3. Plans: `2026-08-12-v1.162-p1-fork-backend-foundation` + `2026-08-12-v1.162-p2-fork-authoring-ui`. |
...
| ~~DF-60~~ | Multi-novel lifecycle (2-step completion + completion-lock + runtime lock columns + `creator works` IA) | **V1.41 P0** (Shipped 2026-06-11) | PR [#53](https://github.com/42ch-dev/nexus/pull/53) merged to `main`; post-merge `12753eb8` lineage validation. Plan [2026-06-10-v1.41-multi-work-switch.md](../plans/2026-06-10-v1.41-multi-work-switch.md). Spec [novel-writing/multi-work-lifecycle.md](../specs/novel-writing/multi-work-lifecycle.md). **Note:** production `runtime_lock_holder` acquire deferred V1.42 P0. |
| ~~DF-61~~ | Selection pool + inspiration pool (DB SSOT + `Pool/Ideas/` MD) | **V1.41 P1** (Shipped 2026-06-11) | PR #53; post-merge `156e669d` `set_pool_active` creator_id authz. Plan [2026-06-10-v1.41-selection-pool.md](../plans/2026-06-10-v1.41-selection-pool.md). Spec [novel-writing/work-pool.md](../specs/novel-writing/work-pool.md). |
...
| ~~BL-10~~ | **Superseded** | V1.46 P1 (2026-06-15) | `docs/novel-writing-quickstart.md` retired. Content migrated to specs only: narrative happy path → [novel-writing/author-experience.md](../specs/novel-writing/author-experience.md) §3; CLI workflow → [creator-run-preset-entry.md](../specs/creator-run-preset-entry.md) (Shipped Master V1.45). Runtime remediation strings updated to cite spec paths. No replacement file; `docs/ARCHITECTURE.md` links to specs. Plan: [2026-06-14-v1.46-spec-cli-hygiene.md](../plans/2026-06-14-v1.46-spec-cli-hygiene.md). |
...
Archived from [deferred-features-cross-version-tracker.md](deferred-features-cross-version-tracker.md) during V1.88 P-last tracker cleanup. IDs may overlap with earlier tracker versions; these rows represent the V2 tracker state as of 2026-07-04.
...
| PF-ESSAY | `essay` Work profile | Shipped V1.63 | Scaffold + `essay-writing` preset + 4-dim rubric + completion + optional KB. Spec: [essay-profile.md](../specs/essay-profile.md). |
| PF-GAME-BIBLE | `game-bible` Work profile | Shipped V1.55 P2 (Master) | `design-writing` + 五问 + section completion + KB extraction. Spec: [game-bible-profile.md](../specs/game-bible-profile.md). |
| PF-SCRIPT | `script` Work profile | Shipped V1.60 P1 (Master) | V1.55 scaffold; V1.60 Depth 3.5: `script-writing` preset + 五问 + completion. Spec: [script-profile.md](../specs/script-profile.md). |
...
| ~~DF-75~~ | **FL-L W5 — Preset injection slots + Moment Directive + generation-type gates** | V1.150 | PD-10 / FL-L wave 2 (spoke absorption W5). **Shipped (P0+P1+P2, 2026-08-05):** **slot engine** (`crates/nexus-moment-context-assembly/src/slots.rs`) routing V1.149-activated lore into named ordered slots within `## World Knowledge Base` — `world.before`/default fallback/`world.after`/`kb.outlet.<name>` (open, sorted)/`style.post_history` (reserved tail outlet); emit order locked (spec §2/Q5); **Moment Directive** — product-local author instruction (body + `insert_depth` head/mid/tail + TTL `generations`/`chapters` + optional `clear_on_scene_change`), `moment_directives` table, per-Work + World-override scope (closest wins), `creator moment-directive set|show|clear` CLI, **never on spoke wire** (AC-I3); **generation-stage gates** — `MomentRequest.generation_stage` (`intake`…`persist` + `work_maintenance`/`system_maintenance`/`unspecified`; `run_intent` derivable, no separate field): `style.post_history` on only for `produce`/`review` (AC-I4), `system_maintenance` runs no lore, `unspecified`/direct CLI all-on. Neutral-only byte-equivalence + no-directive path unchanged (goldens green). Consumer-only — `wire_contracts_changed: false`, no `spoke-operations` changes. Plans: `2026-08-05-v1.150-p0-preset-slot-engine-and-outlet-actioning` + `…-p1-moment-directive-author-note-analogue` + `…-p2-generation-triggers-dogfood-and-sweep`. Dogfood: stage×directive assembly on seeded Worlds + CLI TTL-3 lifecycle + real-binary smoke (`--stage` flag, `system_maintenance` no-lore, unknown-stage safe default) — evidence in V1.150 P2 plan closeout notes. Next wave: DF-76 (FL-L W6 — trigger "DF-75 slot + Directive traces inspectable" met at V1.150 ship; includes deferred thin Control Room Moment Directive input). |
| ~~DF-76~~ | **FL-L W6 — Assembly inspector (Control Room / CLI)** | V1.151 | PD-10 / FL-L wave 3 (spoke absorption W6). **Shipped (P0+P1+P2, 2026-08-05; dogfood green):** enriched inspector packet — `modules.*` keeps the spoke assemble-module recipe vocabulary (consumer-only: presents, does not clone the spoke assemble wire; AC-I3) with `placement[]` = accepted entries `{entry_id, canonical_name, reason}` and `activation_trace[]` = full fire/miss `{entry_id, canonical_name, reason, accepted}` (P2-confirmed against the spoke `assemble-module-recipes.md` handbook), plus three additive **product-local** top-level sections: `slot_map` (entry → slot id, captured post stage-gate), `budget` (chars/4 `primary_tokens_est`/`hop_tokens_est`/`cap`/`remaining`), `moment_directive` (status/metadata only — the directive body never on the wire). Shared builder `crates/nexus-moment-context-assembly/src/inspector.rs` (relocated from `apps/nexus42`); daemon route `POST /v1/daemon/inspector/moment` (tier2, ownership-guarded); CLI `assemble-moment --inspect` / `--emit-packet` (extended); read-only Control Room panel (P1). **Moment Directive Control Room input** — the V1.150-deferred thin set/show/clear item — **closed here** (P0 route `POST /v1/daemon/moment-directive` + P1 UI). No assembled-byte change (AC-I6 — packet is a separate emission path); no new spoke wire. Spec: `spoke-adapter-architecture.md` §7.4 (V1.151 inspector packet field surface). Next wave: DF-77 (FL-L W7 — Knowledge Pack productization). |
| ~~DF-77~~ | **FL-L W7 — Knowledge Pack productization + optional ST lorebook importer** | V1.152 | PD-10 / FL-L wave 4 (spoke absorption W7). **Shipped (P0+P1+P2, 2026-08-06; dogfood green):** daemon pack export/import routes (`POST /v1/daemon/worlds/:world_id/kb/pack/export|import`, ownership-guarded, all three conflict policies skip/rename/overwrite) + shared `import_pack` orchestration (`crates/nexus-daemon-runtime/src/pack_import.rs`) consumed by CLI + daemon; Control Room Pack panel (P1); CLI `rename`/`overwrite` completeness (P0); `modules.activation` + relations round-trip dogfood (`dogfood_pack_round_trip_preserves_activation_and_relations`). Consumer-only handbook transport (`nexus_spoke_adapter::pack`); `pack_import` provenance. Spec: `spoke-adapter-architecture.md` §11. Optional ST lorebook → Pack importer promoted to DF-80 in active tracker §2.4 (2026-08-11). Next optional waves: DF-78/79/80. |
| ~~FEAT-WASM-COMPUTE~~ | **Programmable Narrative Progression — WASM compute for timeline narrative** | V1.61 | Core differentiator shipped in V1.61: wasmtime + KB structured layer + `narrative.compute` + `combat-engine` preset. V2 deferred items (Generic Combat Protocol, CDN distrib, 3P game bridge, marketplace, GPU/SIMD) tracked in DR-49. Compass: [v1.61/delivery-compass.md](../iterations/v1.61/delivery-compass.md). **Moved from §2.3 open → shipped archive (V1.160 tracker hygiene).** |
| ~~DF-V1122-HARNESS-RENAME~~ | **Strategy/Preset → Harness product copy (breaking UX rename)** | V1.156 P3 | User-visible Strategy/Strategies → Harness (pillar entry); Preset stays (mechanism); internal identifiers unchanged (architect LOCKED). zh pillar = English "Harness" proper-noun. STRATEGY.md + CONCEPTS.md + web-ui.md deferrals closed. **Moved from §2.3 open → shipped archive (V1.160 tracker hygiene).** |
| ~~DF-V1123-WORLD-MOMENT~~ | **World Timeline Moment layer (scene-precision within World history)** | V1.156 P1 | World Timeline Moment = read/projection of bound Works' Outline scene/beat data (PD-3; Moments Work-owned); frontend-only (`wire_contracts_changed:false`); fixture-driven (DR-26 tracks WorkOutline wire extension for real scene/beat data). **Moved from §2.3 open → shipped archive (V1.160 tracker hygiene).** |
| ~~DF-V1123-WORK-BRIEF~~ | **Work Timeline Brief layer (world-shape projection for a Work)** | V1.156 P2 | Work-Brief = projection of bound World's Brief (all `block_type=era` via V1.73 `kb/graph`; PD-2 read-only); frontend-only (`wire_contracts_changed:false`). 3×2 Brief/Narrative/Moment matrix complete. **Moved from §2.3 open → shipped archive (V1.160 tracker hygiene).** |
| ~~DF-V1123-ERA-TAXONOMY~~ | **Rich era taxonomy for Brief layer (kingdoms, ages, sub-ages; typed nested time-bands)** | V1.159+V1.160 | **V1.159**: typed nested time-bands on World Timeline Brief layer (era_type freeform + custom/parent_era nesting via Relation); `wire_contracts_changed:false`. **V1.160**: entity creation backend (`patch_entity` create-on-absent, §5.1.2) + era create UI activation + Work Timeline Brief time-band inheritance. Closes R-V1159P1-001 + R-V1159P1-002. `wire_contracts_changed:false`. **Moved from §2.3 open (partial) → shipped archive (V1.160 tracker hygiene).** |
...
| Category | Count | IDs / Notes |
|----------|-------|-------------|
| Dogfood validation | 1 window | V1.164–166 full window (quality loop + mental read + observation authoring) — findings register `.mstar/iterations/v1.167/guides/dogfood-findings-register.md` (2A→1A after PM triage / 2B / 6C / 1 gated) |
| Class-A fixes shipped | 1 | DF-A-02 → `creator register --local` complete local-only bootstrap (P2: T1 510b6797 + T2 743060e4 + fix wave af436226; tri-Approve + qc2 revalidation; PM live acceptance) |
| Dependabot disposition | 3 alerts | #41 reachable-in-artifact (upstream unblock), #42/#43 lockfile-only (libp2p ≥0.57); user accepted deferral; `Cargo.lock` byte-unchanged |
| Gate-notes answered | 3 | DR-70 = N; mental-field authoring = Y (DF-81); rules UI authoring = Y (DF-82) |
| New tracker rows | 3 | DF-81 / DF-82 / DF-83 (dogfood + QC dispositions) |
...
| **Active spec** | [creator-run-preset-entry.md](../specs/creator-run-preset-entry.md) promoted Draft → **Shipped Master V1.45** (P-last T1). |
...
| **Active specs promoted** | [archived/knowledge/novel-findings-maturity.md](../archived/knowledge/novel-findings-maturity.md) (Draft V1.48 overlay) **folded** into [novel-writing/quality-loop.md §9](../specs/novel-writing/quality-loop.md) at P-last. [novel-writing/workflow-profile.md](../specs/novel-writing/workflow-profile.md) header updated to V1.48 Shipped (V1.48 §5.5.2 open-findings prompt injection + §5.5.4 AGENTS.md Layer 2 runtime). |
...
| **Active specs** | [novel-writing/quality-loop.md §2](../specs/novel-writing/quality-loop.md) (F6 6-state lifecycle merged from `findings-lifecycle.md` overlay) + [novel-writing/workflow-profile.md §4.6](../specs/novel-writing/workflow-profile.md) (F###/E### 5-column index schema merged from `narrative-indexes.md` overlay) + §5.5.1 three-state paragraph superseded + [novel-writing/author-experience.md §8](../specs/novel-writing/author-experience.md) (author desk deltas merged) |
| **Draft overlays archived** | [novel-writing/findings-lifecycle.md](../specs/novel-writing/findings-lifecycle.md) → Superseded (V1.49 P-last); [novel-writing/narrative-indexes.md](../specs/novel-writing/narrative-indexes.md) → Superseded (V1.49 P-last) |
...
| **Active specs** | [novel-writing/workflow-profile.md §11](../specs/novel-writing/workflow-profile.md) (cron staggering + auto-chronology normative) + [entity-scope-model.md §5.5](../specs/entity-scope-model.md) (World KB promotion state machine Draft → Normative) |
| **Draft overlays archived** | [novel-writing/cron-staggering.md](../specs/novel-writing/cron-staggering.md) → Superseded (V1.50 P-last T3); [novel-writing/auto-chronology.md](../specs/novel-writing/auto-chronology.md) → Superseded (V1.50 P-last T3) |
...
| **Active specs** | [concurrency.md](../specs/concurrency.md) (per-key advisory lock + per-row OCC normative) + [llm-extract.md](../specs/llm-extract.md) (LLM extraction capability normative) + [novel-writing/quality-loop.md §12](../specs/novel-writing/quality-loop.md) (missing-KB detection + cron interplay) + [entity-scope-model.md §5.5.2](../specs/entity-scope-model.md) (KB delta write scope) + [cli-spec.md §6.2G](../specs/cli-spec.md) (`creator world kb pending --missing-only`) + [world-kb-runtime-architecture.md](../knowledge/world-kb-runtime-architecture.md) (full pipeline normative) |
| **Draft overlays archived** | [concurrency.md](../specs/concurrency.md) → Promoted (V1.51 P-last T3); [llm-extract.md](../specs/llm-extract.md) → Promoted (V1.51 P-last T3); 4 other overlays (quality-loop §12, entity-scope §5.5.2, cli-spec §6.2G, world-kb-runtime-architecture) → folded into Masters |
...
| **Open follow-ups** | `R-V1143P2-DEFER-RELATE` (relate cutover → V1.144 `RelationPort` adapter-extension); `R-V1143P2-ACCEPT-01/02` (promote-reject/merge Surface A, accepted) |
