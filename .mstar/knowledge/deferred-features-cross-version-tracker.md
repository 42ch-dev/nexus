...
| DF-61 | Selection pool / inspiration pool (DB SSOT + MD refs) | V1.36 distill | **V1.41 P1 Implemented (pending QC/QA)** | M | V1.36→V1.41 | Implemented on `feature/v1.41-selection-pool` (4 commits). `novel_pool_entries` (P0 schema) + `inspiration_items` (P1) in `state.db`; inspiration files under `Pool/Ideas/`. Pool promote/archive + inspiration add/list/promote/archive via daemon API + CLI `creator works pool`. Auto-chain `mark_work_completed` updates pool row. 9 hermetic tests pass. **Not** `work_profile: novel-pool`. [novel-work-pool.md](specs/novel-work-pool.md) + plan [2026-06-10-v1.41-selection-pool.md](../plans/2026-06-10-v1.41-selection-pool.md). Will be marked Shipped after QC+QA pass. |
...
| **Selection pool / 灵感池** | Obsidian 选题库 + 灵感池 | **V1.41 P1 Implemented (pending QC/QA)** — DB SSOT + `Pool/Ideas/` | DF-61 |
