...
**Quick status**: **V1.38 Shipped** (2026-06-09) · Latest active compass: [v1.38-multi-chapter-serial-writing-delivery-compass-v1.md](../iterations/v1.38-multi-chapter-serial-writing-delivery-compass-v1.md) (status: Shipped) · FL-E **Shipped in V1.34** · Platform **paused** · V1.35 focus: **CLI IA + critical residual convergence + DF-53 partial shipment** → Done · V1.36 focus: **novel-writing正文产出 UX** (`work_profile: novel`, `Works/<work_ref>/` layout, init preset, chapter pipeline, completion stop) → Shipped (DF-57/58 closed; DF-53 partial; DF-47 conditional; DF-59 backlog) · **V1.37 focus: Novel Writing UX foundation-first** (P0 gate_evaluator + AddScheduleRequest.input + scaffold atomicity + --force-gates audit; P1/P2/P3 spec/roadmap amendments for DF-62, DF-63, DF-64/65/66/67) → Shipped (R-V136P1-01/02, R-V136P3-02 closed; R-V137P0-01 serde strict-mode opened) · **V1.38 focus: DF-62 multi-chapter / serial writing first implementation slice** (P0 chapter selection/status foundation: `next_chapter(work_id)` single MIN query, `current_chapter` finalize-only advance, completion with intake + current_chapter checks, per-chapter CLI status UX, composite index; P1 `novel-writing` selected-chapter parameterization: preset version5→6, `chapter_label`/`outline_path`/`body_path`/`slug` template vars, fail-fast CLI validation, shared `chapter_label()` helper) → Shipped (12 P0+P1 residuals registered: medium=2, low=5, nit=5; **no auto-chain / auto-reenqueue**) · Tech debt SSOT: [`status.json`](../status.json)
...
**Created**: 2026-04-21  
**Last updated**: 2026-06-09 (V1.38 Shipped: P0 chapter selection/status foundation + P1 `novel-writing` selected-chapter parameterization landed; DF-53 auto-chain, DF-63 World KB, DF-64/65/66/67 quality loop, multi-volume PK migration, platform publish, multi-work switch, and selection pool remain deferred)
...
| DF-53 | FL-E `--auto-chain` default stage sequencing | V1.34 | V1.38 explicitly OUT; V1.39+ exploration | S | V1.34→V1.35→V1.36→V1.37→V1.38 | V1.35 P4 partial **shipped**: `--chain-novel-writing` defaults true (intake → produce); clap opt-out syntax `--chain-novel-writing=false` works. V1.36 P2 partial **shipped**: layout-aware chain (Works/<work_ref>/) cross-linked in cli-spec §6.2D + novel-writing preset gates (§5.3.2). V1.37 P0 improved fail-fast/remediation UX through gate evaluation. **V1.38 does not include automatic continuation / auto-reenqueue**; status may show the next action, but full multi-stage auto-chain (research → produce → review → persist) remains open until explicitly implemented. |
...
| DF-62 | Multi-volume auto-chronology (per-volume outline + chapter range tracking) | V1.36 distill | **V1.38 P0/P1 target: multi-chapter / serial writing first implementation slice; multi-volume PK migration deferred** | M | V1.36→V1.37→V1.38<br>V1.37 P1 — spec-only amendment; implementation deferred<br>V1.38 prepare — P0/P1 implementation plans opened | V1.37 P1 plan [2026-06-07-v1.37-novel-multi-chapter-chronology.md](../plans/2026-06-07-v1.37-novel-multi-chapter-chronology.md) amended [novel-workflow-profile.md §4.5](specs/novel-workflow-profile.md#45-multi-chapter-and-multi-volume-semantics-v137-extension) and [§8.1](specs/novel-workflow-profile.md#81-multi-chapter-creator-run-status-ux-v137-extension). V1.38 targets the first executable slice via [2026-06-08-v1.38-multi-chapter-selection-status.md](../plans/2026-06-08-v1.38-multi-chapter-selection-status.md) and [2026-06-08-v1.38-novel-writing-parameterization.md](../plans/2026-06-08-v1.38-novel-writing-parameterization.md): DB-selected chapter 2..N, `next_chapter(work_id)`, `current_chapter`, completion/status UX, tests, and selected-chapter prompt inputs in one `novel-writing` preset. Multi-volume duplicate chapter numbering / PK migration remains deferred until a later plan explicitly reopens it. Open until implementation ships. |
| DF-63 | **World KB cross-Work unification** (was: Worldbuilding 7 sub-categories schema) | V1.36 distill → V1.36 spec §3.5 refactor | **V1.37 P2 roadmap-only; implementation deferred to V1.37+ when P0/P1 stable and a future plan reopens** | L | V1.36→V1.37<br>V1.37 P2 — spec-only roadmap; 5+ implementation work items deferred (creator world create, KB taxonomy schema, World KB query, world_refs validation, kb-extract binding) | V1.37 P2 plan [2026-06-07-v1.37-novel-world-kb-roadmap.md](../plans/2026-06-07-v1.37-novel-world-kb-roadmap.md) records the audit and roadmap-only scope decision. Spec extensions: [entity-scope-model.md §5.1.1](specs/entity-scope-model.md#511-narrative-world-kb-item-taxonomy-v137-p2-roadmap) defines the narrative World KB taxonomy and minimum fields; [novel-workflow-profile.md §3.5.1](specs/novel-workflow-profile.md#351-world-kb-continuity-roadmap-v137-p2-extension) defines `creator world create`, `world_id` validation, prompt World context block, `world_refs` validation, and Work chapter正文 → World KB extraction. **V1.36 refactor** remains: per-Work `Worldbuilding/` subtree is removed; world content lives in **World KB**. Open until implementation ships. |
| DF-64 | Findings lifecycle (review → brainstorm → write coordination; 3-role) | V1.36 distill | **V1.37 P3 roadmap-only; implementation deferred to V1.37+ when a future plan reopens** | L | V1.36→V1.37<br>V1.37 P3 — spec-only; implementation deferred | V1.37 P3 plan [2026-06-07-v1.37-novel-quality-loop-roadmap.md](../plans/2026-06-07-v1.37-novel-quality-loop-roadmap.md) amended [novel-workflow-profile.md §5.5](specs/novel-workflow-profile.md#55-quality-loop-roadmap-v137-p3-extension). Spec defines findings lifecycle, severity mapping, and local DB sketch; implementation remains open as part of the five-item quality-loop roadmap: findings table/API/status, future brainstorm/master presets, 96h scheduled banner, rules files/history, and logs/reflection integration. Findings backplane must be local DB, not Redis. |
| DF-65 | Three-layer rules architecture (`writing-craft-rules.md` / `novel_rules.md` / `novel_rules_history.md`) | V1.36 distill | **V1.37 P3 roadmap-only; implementation deferred to V1.37+ when a future plan reopens** | M | V1.36→V1.37<br>V1.37 P3 — spec-only; implementation deferred | V1.37 P3 spec amendment [novel-workflow-profile.md §5.5.4](specs/novel-workflow-profile.md#554-three-layer-rules-architecture) defines shared craft rules, per-Work `Rules/novel-rules.md`, and append-only `Rules/novel-rules-history.md`, while keeping SOUL and World KB responsibilities separate. Implementation remains open as part of the same five-item roadmap (findings, presets, timeout, rules, logs/reflection). |
| DF-66 | Per-chapter log subdirectories (write/review/brainstorm/publish at `Works/<work_ref>/Logs/`) | V1.36 distill | **V1.37 P3 roadmap-only; implementation deferred to V1.37+ when a future plan reopens** | S | V1.36→V1.37<br>V1.37 P3 — spec-only; implementation deferred | V1.37 P3 spec amendment [novel-workflow-profile.md §5.5.5](specs/novel-workflow-profile.md#555-logs-structure-and-write-discipline) defines `Logs/brainstorm`, `Logs/write`, `Logs/review`, and future `Logs/publish` write discipline. `Logs/**` remains excluded from chapter sync; chapters live under `Stories/`. Implementation remains open as part of the five-item quality-loop roadmap. |
| DF-67 | Master-decision timeout (96h finding escalation) | V1.36 distill | **V1.37 P3 roadmap-only; implementation deferred to V1.37+ when a future plan reopens** | S | V1.36→V1.37<br>V1.37 P3 — spec-only; implementation deferred | V1.37 P3 spec amendment [novel-workflow-profile.md §5.5.3](specs/novel-workflow-profile.md#553-master-decision-timeout-96h-in-local-first-oss-terms) maps the 96h escalation to local DB + `nexus-daemon-runtime` scheduled lifecycle task + `creator run status` banner; automatic review-master action is opt-in. Implementation remains open as part of the five-item quality-loop roadmap; no Redis, external cron, or platform worker dependency. |
...
- **V1.38** (Active 2026-06-08): [v1.38-multi-chapter-serial-writing-delivery-compass-v1.md](../iterations/v1.38-multi-chapter-serial-writing-delivery-compass-v1.md) — **DF-62 multi-chapter / serial writing implementation readiness**: P0 targets `next_chapter(work_id)`, DB-selected chapter 2..N, `current_chapter`, completion/status UX, and tests; P1 targets selected-chapter parameterization for one reusable `novel-writing` preset. No automatic continuation / auto-reenqueue; DF-53 full auto-chain, DF-63 World KB, DF-64/65/66/67 quality loop, multi-volume PK migration, platform publish, multi-work switch, and selection pool remain deferred.
...
*Last updated: 2026-06-08 (V1.38 Active). Status: **V1.38 Active**; V1.37 Shipped 2026-06-08. V1.38 targets DF-62 through P0 chapter selection/status foundation and P1 `novel-writing` selected-chapter parameterization. DF-53 auto-chain remains deferred; V1.38 may show next action but does not auto-reenqueue. DF-63 and DF-64/65/66/67 remain deferred until a future plan explicitly reopens them. Multi-volume PK migration, platform publish, multi-work switch, and selection pool remain out.*
