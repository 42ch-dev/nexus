...
| DF-63 | **World KB cross-Work unification** (was: Worldbuilding 7 sub-categories schema) | V1.36 distill → V1.36 spec §3.5 refactor | **V1.40 Active** (P0 W1+W4 shipped; P1–P3 pending) | L | V1.36→V1.37→V1.40 | V1.37 P2 roadmap shipped as spec; **V1.40** implements via [v1.40-novel-world-kb-delivery-compass-v1.md](../iterations/v1.40-novel-world-kb-delivery-compass-v1.md) + [world-kb-runtime-architecture.md](world-kb-runtime-architecture.md). **W1 (world create) + W4 (world_refs validation) Shipped V1.40 P0** via [2026-06-10-v1.40-world-create-and-validation.md](../plans/2026-06-10-v1.40-world-create-and-validation.md): `creator world show`, `--title`/`--name` alias, scaffold `create_world` atomic path, `world_id` FK validation with remediation on Work PATCH, `world_refs` validator (outline warning / finalize error / worldless warn-only). Remaining W2 (taxonomy body validation) / W3 (prompt context block) / W5 (kb-extract binding) targeted P1–P3. Close row on P3 ship. |
