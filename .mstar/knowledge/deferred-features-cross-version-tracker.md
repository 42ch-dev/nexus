...
| FEAT-WORLD-KB-RELATIONSHIPS | World KB relationships surface (`world_kb.patch_relationship` + `kb_relationships` table) | V1.73 | V1.74 (`tbd-v1.74-world-kb-relationships`) | L | V1.73 | V1.73 World KB β shipped entity + promotion patch routes but deferred relationship edges (no `kb_relationships` table exists). V1.73 renders derived reference/source-anchor edges read-only. V1.74 introduces the relationships table + `world_kb.patch_relationship` route + relationship-type semantics (directed/typed/confidence). Prerequisite: architect grill-me on relationship-type semantics. |
