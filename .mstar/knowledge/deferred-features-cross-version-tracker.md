...
**Quick status**: **V1.58 Active (2026-06-22)** — Workspace OCC Hardening & DF-44 Reference Refresh (S-A + S-B dual track; 3-wave dispatch); 7 plans registered on integration branch `iteration/v1.58` (PM-locked grill decisions; pre-implement gate=go). Track A (P0+P2): absorb 33 V1.57+/V1.58+ residuals (workspace OCC hardening + capability surface quality + DF-56 polish + V1.57-new closures); Track B (P1+P3): DF-44 reference body refreshable pipeline (`nexus.reference.refresh` capability + daemon refresh-scheduler + `nexus42 reference refresh` CLI + DB migration). V1.57 Shipped 2026-06-22 via PR #79 @ 76e02d41; `iteration/v1.57` retired. V1.56 Shipped 2026-06-21 via PR #78 @ 8a2fb20e. Platform **paused**. Tech debt SSOT: [`status.json`](../status.json)

**Status**: V1.58 Active (P-1 merged 2026-06-22; pre-implement gate go). S-A + S-B dual track with 3-wave dispatch (V1.51-V1.57 rhythm). Track A: P0 `2026-06-22-v1.58-workspace-occ-hardening` (L effort, `@fullstack-dev`) absorbs 19 V1.57+/V1.58+ residuals; P2 `2026-06-22-v1.58-capability-quality-convergence` (M effort, `@fullstack-dev`) absorbs 14 V1.57+ residuals. Track B: P1 `2026-06-22-v1.58-df44-reference-refresh-pipeline` (L effort, `@fullstack-dev-2`) ships `nexus.reference.refresh` capability + acp §4 roster extend (41→~43 rows) + DB migration; P3 `2026-06-22-v1.58-reference-cli-and-cross-cut-tests` (M effort, `@fullstack-dev-2`) ships `nexus42 reference refresh [work_ref|all]` CLI + cross-reference E2E test. P-mid + P-last meta/closeout. V1.57 Shipped 2026-06-22 via PR #79 @ 76e02d41 on main; integration branch `iteration/v1.57` retired per `mstar-branch-worktree`.

**V1.58 carry-forward index**: 33 of 35 V1.57+/V1.58+ open residuals absorbed. Track A: P0 absorbs 19 (R-V156P0-M001..M006 workspace OCC; R-V156P1-M003..M005 capability surface; R-V156P1-L001..L007 polish; R-V156-PROCESS-01 + R-V156P1-CACHE-01 sqlx hygiene; R-V156-MIDQA-01 fmt drift; R-V157P0-L001/L002 V1.57-new). P2 absorbs 14 (R-V156P2-M001..M003 + L001..L004 DF-56 independent; R-V156P2-CACHE-01 engine test fidelity; R-V156P3-W001/W002 + S001/S002/S004 DF-56 dependent; R-V157P1-W001 host-call smoke). Track B: DF-44 closure split P1 (capability + DB migration + new `reference-knowledge.md` Draft spec) + P3 (CLI + cross-cut tests + topology update). **OUT of V1.58 scope** (deferred to V1.59+): V1.52-era WL-A polish residuals (14 items: R-V150KBED-02, R-V151Q1-10, R-V152TA-S001..S011, R-V152TB-W006/W007/W008) — unrelated to OCC/DF-44 scope.
...
| DF-44 | Reference body externalization — refreshable scan pipeline | V1.26 | **V1.58 P1+P3** | M | V1.26→V1.58 | Static registration shipped V1.26. V1.58 P1 `2026-06-22-v1.58-df44-reference-refresh-pipeline` ships `nexus.reference.refresh` capability + DB migration (extend `reference_sources` with `last_refreshed_at` / `refresh_policy` / `refresh_status` columns) + new Draft spec `reference-knowledge.md` + acp §4 roster extend (41→~43 rows). V1.58 P3 `2026-06-22-v1.58-reference-cli-and-cross-cut-tests` ships `nexus42 reference refresh [work_ref|all]` CLI + daemon refresh-scheduler hook integration + cross-reference E2E test. **Closed in V1.58**; row archived at V1.58 P-last. |
