...
**Quick status**: **V1.147 active (in-flight)** — direct compute lane shipped: Run Studio + compute-on-Timeline (DF-V1122-COMPUTABLE-UI + DF-V1122-COMPUTE-ON-TIMELINE closed → shipped archive); DF-V1122-DEEPER-WB event-row read slice landed (row open). Platform **paused**. **Backlog themes (2026-08-02):** FL-R Connect/`nexus-runtime` (PD-09; DF-72/73); **FL-L** SillyTavern lore-activation Phase B (PD-10; DF-74–79) — SPOKE Phase A complete on `0.6.1`.
...
**Created**: 2026-04-21 · **Last updated**: 2026-08-02 (PD-09/10 + FL-R/FL-L + DF-72–79 recorded)
...
| DF-72 | Cross-cutting | **SPOKE Connect Host — Adapter-full invoke surface** | 2026-08-02 | Backlog (FL-R) | L | PD-09 / FL-R. Nexus Connect Host: explicit peering + WS (default; align with `@42ch/spoke-connect-ts`) + session core; `ConnectInvoke` → `NexusAdapter` `orchestrate_*` (upsert / promote / relate / check / assemble / project / compute as capabilities allow). Honest `HostCapabilityManifest`; capability-token + world/tenant scoping. Phased: N-C0 manifest → N-C1 write ops → N-C2 check/assemble production → N-C3 `list_peer` / multi-host. **Blocked on:** spoke connect Stage 1 publish (integrator client); Nexus `RuleQueryPort` + `orchestrate_check` production cutover for “reasoning-complete” claim. Coexists with Daemon HTTP (creator UI SSOT). **Owner:** architect. **Trigger:** spoke `@42ch/spoke-connect-ts` published + partner demand for headless narrative ops. |
