...
| [architecture-patterns/nexus-brand-token-hierarchy.md](architecture-patterns/nexus-brand-token-hierarchy.md) | Nexus brand & design token hierarchy — root DESIGN pair is the sole token SSOT → `@nexus/design-tokens` shared Tailwind preset + tokens.css → `@42ch/nexus-ui` brand → app implementation; LFS PNG + SVG asset policy; **Chronos dual-role**; **V1.132 P2** theme-split primary + plain vs `*-square`; **V1.135 P1** macOS Dock icon = opaque RGB + baked squircle plate (not full-bleed-only); author Dock confirm hard gate (V1.83–V1.135) |
