...
| [architecture-patterns/complete-host-retirement-method.md](architecture-patterns/complete-host-retirement-method.md) | Complete host retirement method — evidence-first disposition ledger, the closed-operation rule (ports + observation + forced capability), test-migration receipts, cohort collapse honesty (v1.193 daemon/legacy-host retirement distilled) |
...
| [architecture-patterns/resolved-residual-verification.md](architecture-patterns/resolved-residual-verification.md) | **Updated (v1.182)** — aged-cluster re-triage playbook appended: rebuild inventory from archived carriers, adjudicate-before-touch disposition table, bounded fix slice + honest roadmap remainder. Base: residual lifecycle is a claim, not a guarantee — verify against current `main` HEAD (V1.86 / V1.93 symmetric case) |
...
| [architecture-patterns/native-cli-provider-adapter-pattern.md](architecture-patterns/native-cli-provider-adapter-pattern.md) | Native provider SDK ownership, verified candidate→ready health, safe diagnostics, message-level dsh streaming and retained cleanup; ACP bare-command extraction/dedup. Updated from verified V1.188 P0–P4; public first-run/live composition deferred. |
...
| Document | Description |
| --- | --- |
| [conventions/crash-resilient-subagent-report-dispatch.md](conventions/crash-resilient-subagent-report-dispatch.md) | **Updated (v1.182)** — provider-exhaustion failover appended (verify zero residue → one corrected retry → track switch; woken-corpse hazard). Base: long-running dispatches write report skeletons FIRST + append incrementally (V1.164–V1.165) |
...
| [architecture-patterns/stateless-mcp-bridge-child.md](architecture-patterns/stateless-mcp-bridge-child.md) | Stateless MCP bridge child (Model A) — client-spawned `nexus42 mcp serve` stdio proxy: no registry/policy/cache, live daemon round trips, lockstep by construction; rmcp 3.2.0 boundary (runtime-dynamic `ServerHandler`, `CallToolResponse` wrapper, preserved empty-list defaults and two-class error mapping), schema mapping, alternatives → DF-87/88 (compound V1.174; dependency-sweep SDK update) |
...
| Document | Description |
| --- | --- |
| [architecture-patterns/flow-runner-resume-re-drive.md](architecture-patterns/flow-runner-resume-re-drive.md) | BL-04 checkpoint slice (V1.180) — resume = filter + re-drive over graph-flow's existing per-task save/load + boot recovery; class checks must test values not key presence (graph_flow Context::set never removes keys; Null = finished); typed-fail filter keys off `_run_status`/`_run_error` (DB status stays 'running' forever); boot background work needs CancellationToken + shutdown-notify watcher; `_join_wait_start_*` elapsed includes downtime (compound V1.180) |
| [workflow-patterns/branch-diff-vs-worktree-state.md](workflow-patterns/branch-diff-vs-worktree-state.md) | Review-package diffs (commit-derived) cannot see uncommitted working-tree corruption — a patch-identical `git diff` still passed cargo check over a duplicated boot.rs paste block; protocol: worktree-scoped reviewers run `git status --short` + echo HEAD; seat disagreement → PM runs cheap probes (`grep '^+'`, cargo check) to adjudicate instead of re-dispatching reviewers (compound V1.180) |
| [architecture-patterns/peer-tool-registration-exposure-lanes.md](architecture-patterns/peer-tool-registration-exposure-lanes.md) | **Updated** — V1.180 RN-OGA-2 visibility≠authorization split: `VisibilityPolicy` at the two MCP construction sites (wire-free), execution authz stays spine-owned (L0–L3); hidden-tool = seam-minted `NotAuthorized` → `tool_not_authorized` discriminator before backend vs visible-but-denied = spine-shaped refusal; `mcp_visibility` boot-scoped with `restart_required` signal |
| [architecture-patterns/additive-packet-sections-are-wire-changes.md](architecture-patterns/additive-packet-sections-are-wire-changes.md) | Additive packet sections on a closed (deny_unknown_fields) DTO ARE wire changes — schema + codegen + additive version bump required; enumerate validating DTOs when writing `wire_contracts_changed: false`; gate claims must cover E2E suites, not lib subsets; plan writeback at discovery (v1.181 P1 T1 distilled; compound v1.181) |
| [best-practices/clean-room-importer-stable-ids-diagnostics.md](best-practices/clean-room-importer-stable-ids-diagnostics.md) | Clean-room importer guidance: stable ids from format-intrinsic fields through a single unique-claim helper (positional fallback + collision suffixing); two-tier error taxonomy (file-level abort, per-entry diagnostics printed before summary); never silently map/drop foreign semantics; test the id-stability property and the exact collision repro (v1.181 P1 DF-80 distilled; compound v1.181) |
...
| [testing-patterns/behavior-first-assertions-shared-ui.md](testing-patterns/behavior-first-assertions-shared-ui.md) | Behavior-first assertions for shared UI — assert consumer-observable contracts (roles, announcements, focus/state, scoped queries) instead of exact copy or implementation class strings; class pins are legitimate only when the class name is the token contract, and then every cross-package pin site is swept; delete tautologies instead of keeping them green (v1.187 all-plan fix-round class distilled; compound v1.187) |
| [ui-bugs/studio-sticky-header-wrapped-overlap.md](ui-bugs/studio-sticky-header-wrapped-overlap.md) | Studio sticky header wrapped-overlap — a fixed-height sticky header whose nav wraps covers the first content row at 390px (pointer hit-testing fails while DOM `.click()` passes); fix = minimum height + measured `--studio-sticky-header-offset` consumed by `.scroll-mt-sticky-header` (v1.187 P2 distilled; compound v1.187) |
| [ui-bugs/iframe-reset-remount-by-key.md](ui-bugs/iframe-reset-remount-by-key.md) | Iframe reset/retry must remount by key — recomputing an identical frame URL leaves the document mounted, so fixture state survives Reset and a timed frame cannot recover; bind each iframe to `key={effectiveResetKey}` and prove remount with a marker on the frame window (v1.187 P2 distilled; compound v1.187) |
| [test-failures/jsdom-resizeobserver-unconditional-construction.md](test-failures/jsdom-resizeobserver-unconditional-construction.md) | jsdom regressions from a browser-only API constructed unconditionally — `new ResizeObserver(...)` in a mount effect failed 181 jsdom tests; fix = run the initial measurement outside the capability guard and guard only the construction/observation; do not stub the global (v1.187 P2 distilled; compound v1.187) |
| [test-failures/reader-footer-leak-into-written-file.md](test-failures/reader-footer-leak-into-written-file.md) | Reader-footer leak into a committed file — a test file re-written from a paginated read output shipped the reader's `[Showing lines …]` footer as its last line and stopped parsing; repair in a dedicated commit plus a diff-wide marker sweep; never re-assemble file content from a partial read (v1.187 P1 distilled; compound v1.187) |
| [architecture-patterns/nexus-brand-token-hierarchy.md](architecture-patterns/nexus-brand-token-hierarchy.md) | **Updated** — re-based on v0.5: cobalt/silver-neutral/graphite values, ink-vs-signal roles, background-driven contrast invariant, offline system typography, compiler-owned projection and the four-tier/structural namespace rules; v0.4 warm-paper/serif/cyan sections retired (compound v1.187) |
| [architecture-patterns/editorial-typography-voice-split.md](architecture-patterns/editorial-typography-voice-split.md) | **Updated** — v0.5: content voice is the larger sans `display-*` tier, not a serif; the split (creative-entity titles vs chrome) and the `CardTitle.voice` prop survive with tier-based enforcement (compound v1.187) |
| [architecture-patterns/self-hosted-ofl-font-wiring.md](architecture-patterns/self-hosted-ofl-font-wiring.md) | **Updated** — v0.5: no self-hosted font is active; the pattern is retained as the reintroduction procedure, amended for the generated `tokens.css` (never hand-edit; emit through the compiler or a separate shared CSS layer) (compound v1.187) |
| [architecture-patterns/component-variant-token-projection.md](architecture-patterns/component-variant-token-projection.md) | **Updated** — the four-piece pipeline now starts from DESIGN-sourced projection entries; the gate is regeneration + byte-compare instead of a handwritten needle list (compound v1.187) |
| [architecture-patterns/ui-component-promotion-workflow.md](architecture-patterns/ui-component-promotion-workflow.md) | **Updated** — V1.187 extension: four source tiers (`promoted` / app presentational extract / transitional / studio-local), exact-root/subpath boundaries, locked precedence, fail-safe fallback, catalog+docs lockstep, and the boundary regression tests (compound v1.187) |

### v1.190 additions

| Document | Description |
| --- | --- |
| [architecture-patterns/typed-authority-carriers-and-fences.md](architecture-patterns/typed-authority-carriers-and-fences.md) | Typed authority carriers and fence leases — mint admitted identity as an opaque non-`Deserialize` value (`AdmittedActor`) so a serialized projection can never replay as authority; fence effects with a two-layer all-`try_*` lease (per-service `tokio` RwLock + stable per-entity OS lock file) so busy is observable in-process and across processes; re-read stored epoch under the fence and write the committed epoch back onto the held lease; single ownership as an RAII `OwnerReservation` (failed/panicking build never fences its own DB) plus an established-owner slot for started authorities; one owner per session index (v1.190 core extraction; compound v1.190) |
| [conventions/deliberate-guard-scope-and-clippy-allow.md](conventions/deliberate-guard-scope-and-clippy-allow.md) | Deliberately wide guard scopes and lint suppression — `significant_drop_tightening` must be suppressed at **fn level**: a statement-level `#[allow]` does not suppress it (verified clippy 0.1.98); classify each finding as real (fix) vs required critical section (suppress with the invariant named); never `#![allow]` at crate root; distinguish suppression from a genuine lock-order/`await_holding_lock` defect (compound v1.190) |
| [workflow-patterns/scoped-cargo-target-dir-per-worktree.md](workflow-patterns/scoped-cargo-target-dir-per-worktree.md) | Scoped `CARGO_TARGET_DIR` per feature worktree — `.envrc` auto-detects checkout context (main/integration share the canonical dir, each feature worktree gets `nexus-target-<dirname>`); merge gate deletes that feature's cache before the branch merges (six parallel targets consumed 98 GiB in v1.190); never set the dir in `Cargo.toml`/`.cargo/config.toml`/user config (compound v1.190) |
| [build-errors/duplicate-registry-symbol-across-crates.md](build-errors/duplicate-registry-symbol-across-crates.md) | Same-named registry methods in two crates break cross-crate text rewrites — a partially migrated workspace holds `ActorSessionRegistry`/`CharacterOperationSnapshot`/`reserve_character_operation` in both `nexus-core` and `nexus-daemon-runtime` with **diverged** signatures, so a tree-wide rewrite of the call shape is correct for one crate and produces `E0308 expected Snapshot, found &Snapshot` for the other; enumerate definitions (not call sites) first, scope the edit per crate, and schedule the transitional copy's deletion (Bug track; compound v1.190) |
| [design-patterns/response-scoped-generated-type-qualification.md](design-patterns/response-scoped-generated-type-qualification.md) | Response-scoped type qualification for generated DTOs — `typify` embeds a sibling copy per `$ref` target, so the crate-root barrel and the owning response module expose the same type name as distinct Rust types (80 such names at this revision); import the whole family from the composite's module when constructing it, pick the path by operation (construct → response module, name-only → crate root), and leave the generator dedupe to the codegen lane (compound v1.190) |
| [tooling-decisions/sanitizer-returning-path-validators.md](tooling-decisions/sanitizer-returning-path-validators.md) | Sanitizer-returning validators for taint-tracked path guards — a `validate_x(id) -> Result<(), _>` verdict-only guard is invisible to CodeQL taint modelling, so `rust/path-injection` fires on every later `Path::join`; the delivered shape (v1.194 P1) returns the validated value as a **borrow** (`sanitize_entry_id(id) -> Result<&str, _>`, no allocation) and every call site consumes it by rebinding/shadowing the raw binding — consumption is a **call-site convention**, not a compile-time guarantee (a caller that ignores the result still compiles), and **no analyzer recognition is established** (analyzer evidence for the changed revision is an open residual), so the restructure is justified as code hygiene first; if a dismissal is unavoidable, attach an owner + roadmap entry and never blanket-exclude the rule (compound v1.190) |

### v1.191 additions

| Document | Description |
| --- | --- |
| [engineering-conventions/cargo-update-precise-non-idempotent.md](engineering-conventions/cargo-update-precise-non-idempotent.md) | `cargo update -p <crate> --precise` is not idempotent — repeating it toggles in-range companions (prost-derive→itertools, tempfile→getrandom observed); after a checkpoint freezes Cargo.lock, verification is read-only (`--locked` metadata/tree/graph-pin checks) plus a lock-vs-baseline diff, and only the named custodian re-resolves, intentionally (v1.191 P0 T1 distilled; compound v1.191) |
| [conventions/pnpm-toolchain-pin-and-supply-chain-age.md](conventions/pnpm-toolchain-pin-and-supply-chain-age.md) | **Updated** — 2026-09-18 recurrence: the 720-minute minimumReleaseAge window applies to first-party lockstep releases too, and pnpm 11's verify-deps-before-run preflight makes every `pnpm run <script>` fail (not just install); CLI-only env override during the window, CI `--frozen-lockfile` waits it out (v1.191 P1 T1/T2 distilled) |

### v1.192 additions

| Document | Description |
| --- | --- |
| [architecture-patterns/electron-desktop-trust-boundary.md](architecture-patterns/electron-desktop-trust-boundary.md) | Electron desktop trust boundary — one typed host contract (versioned closed envelope, complete-frame bounds, typed operation union); sender validation bound to live window + main frame + current generation; single-channel IPC registration serialized with exactly-once disposal; `nexus://app` scheme resolved on raw decoded components with protocol-before-window ordering; frozen CSP rebuilt per response from the active origin; deny-by-default navigation with one pre-parse external-URL predicate; main-only secret injection for the exact origin + `/v1/daemon/` prefix with a durable cleared tombstone; preload constants locked by a compiled-artifact parity test (v1.192 P0 distilled; compound v1.192) |
| [tooling-decisions/unsigned-macos-packaging-lane.md](tooling-decisions/unsigned-macos-packaging-lane.md) | Unsigned macOS packaging lane — zero-credential success as the default path; signed/release requests and signing env variables reject before effects; packager integrity-digest mutation disabled and PATH sentinels prove zero signing-tool calls; fail-closed preflight with named errors + exact remediation (packaging never builds its own inputs); atomic publish with fsync and previous-output preservation; closed versioned receipt + separate app-file manifest; read-only verifier that fails on a missing min-OS load command; native-runner CI matrix with per-arch artifacts and honest CI-only markers (v1.192 P1 distilled; compound v1.192) |
| [design-patterns/fence-guarded-destructive-primitive.md](design-patterns/fence-guarded-destructive-primitive.md) | Fence-guarded destructive primitive — fence every target through the existing migration admission lock before the first deletion (fail-fast `owner_busy`); admit store directories and file identities via descriptor walk (`O_NOFOLLOW` + `(st_dev, st_ino)`) and unlink relative to the admitted directory so rename/symlink swaps cannot redirect deletion; deterministic post-fence race tests with red→green proof; discriminated `{confirmed\|cancelled}` outcome threaded across the native→JS→IPC→UI seam so cancel is never success; documented residual bounds (fence-by-path drift, non-Unix arm) (v1.192 P0-T3R distilled; compound v1.192) |
| [workflow-patterns/evidence-before-retirement.md](workflow-patterns/evidence-before-retirement.md) | Evidence before retirement — consumer inventory against current HEAD `file:line` before declaring obsolescence; dormant CLI rows adjudicated (callable handlers exist) instead of silently deleted; deferrals carry named blocker + trigger + tracking; closeout ledger (retired / verified retained / deferred / documentation-lag); doc+CI+lockfile sweep with mention classifications and link checks; debris grep for live callers of deleted paths; external PR closures gated on verified main merge (v1.192 P2 distilled; compound v1.192) |
| [workflow-patterns/capability-parity-receipt.md](workflow-patterns/capability-parity-receipt.md) | Capability-parity receipt — frozen stable capability IDs; per-row layer-bound verdicts ("demonstrated" = only the named exercised layer; `[UNVERIFIED]` = missing observation; partial stays partial; deviations vs explicit non-additions); read-only reconciliation over a report+revision+command evidence registry; never sum test counts; closures authoritative over row text; authorization vs evidence with no history backfill; qualification-limits section + ownership handoff (v1.192 P0-T9 distilled; compound v1.192) |

### v1.194 additions

| Document | Description |
| --- | --- |
| [database-issues/keyset-cursor-total-order-key-agreement.md](database-issues/keyset-cursor-total-order-key-agreement.md) | Keyset cursor and in-process merge key must share one truncating rule — SQL's `strftime('%f')` **rounds** the stored sub-millisecond fraction while chrono `timestamp_millis` **truncates**, so the cursor written from the SQL key sat one millisecond above the key the Rust merge sorted a row by and the next page's `(key, id) > (cursor, ?)` excluded it permanently (short final page, `has_more` false while a row remained); repair = one `fraction_millis` truncating helper for row and cursor key plus a row-value cursor comparison, proven by a pinned-fraction, id-assigned boundary regression that is red before and green after at four test threads (v1.194 P2 T5 distilled; compound v1.194) |
| [tooling-decisions/lint-baseline-in-a-degraded-checkout.md](tooling-decisions/lint-baseline-in-a-degraded-checkout.md) | A lint/warning baseline is only a baseline if measured in a healthy checkout — the observed 88-warning target decomposed into 55 environment-independent findings + 33 main-checkout artifacts from dangling `@types/node` links (type-aware program creation failed for three projects, so `error`-type families fired) + 1 build-order row; detect the degraded state with `tsc -p` / `tsgolint --list-files` / link targets before sizing work, re-anchor the baseline, repair the build order in CI instead of overriding, keep the root invocation as the gate, and retain hash-pinned raw captures (v1.194 P0 T1 distilled; compound v1.194) |
| [workflow-patterns/host-engine-identity-and-retired-bucket-closure.md](workflow-patterns/host-engine-identity-and-retired-bucket-closure.md) | Host identity vs engine coordinator identity, and residuals in retired plan buckets — a `binding-invalid` model-handoff refusal is scoped to one checkpoint while every coordinator-envelope path (phase projection, Phase-2 bind, per-plan leases) still works; read the host session id from the host tool, pass explicit ids (distinct per plan row), never falsify an envelope or hand-edit a ledger; and no exposed writer can close a residual in a retired plan's bucket (`plan residual-close` is bucket-scoped), so a verified inherited fix needs a carry-forward entry with the register/product divergence named (v1.194 entry and residual rounds distilled; compound v1.194) |
| [integration-issues/tool-output-markers-written-into-tracked-doc.md](integration-issues/tool-output-markers-written-into-tracked-doc.md) | Tool output written back into a tracked doc — `STRATEGY.md` shipped with `[…23ln elided…]` replacing 23 Decision Log rows plus a `Read artifact://…` EOF footer; Markdown parses it and the structure linter passes, so the content loss is silent — restore byte-for-byte from the last good revision, prove it with a diff that shows only intended edits, keep the restoration in its own hunks, and run a marker grep + tail check + previous-revision diff after any agent-assisted write (v1.194 P3 T1 distilled; compound v1.194) |

### v1.195 additions

| Document | Description |
| --- | --- |
| [architecture-patterns/cancellation-safe-confirmed-close-settlement.md](architecture-patterns/cancellation-safe-confirmed-close-settlement.md) | Cancellation-safe confirmed close — one retained settlement owner per boundary (claim and handoff in one synchronous region; settlement in its own task so caller cancellation or a panic cannot strand it) with waiters that register before re-checking and a fixed report → boundary → notify publication order; the drain set covers admitted durable commits (a commit outlives its caller) and releases the workspace authority in the close path; retries reuse the same single-flight boundary under the same outer budget and stay interrupted until the real settlement; retained-composition `Weak` edges; unconfirmed close is a first-class result with its pending set (v1.195 P0 distilled; compound v1.195) |
| [architecture-patterns/pinned-canonical-root-across-authority-planes.md](architecture-patterns/pinned-canonical-root-across-authority-planes.md) | One pinned canonical root per hosted owner — admit the selection once and hand the pin to the Host probe and the workspace ports; refuse at the pin what a `String`-typed port cannot carry losslessly (no `to_string_lossy`, no U+FFFD substitution into a possibly-existing path); classify a *moved* selection as stale admission before asking whether a root is *representable*, with one shared reader body; a missing pin yields no probe owner instead of a fabricated boundary (v1.195 P0 distilled; compound v1.195) |
| [architecture-patterns/principal-bound-run-event-subscription.md](architecture-patterns/principal-bound-run-event-subscription.md) | Principal-bound same-run event subscription — authorize the stored owner before the ring or cursor is touched (unknown/foreign/child ids close identically, before any SSE header); keep `<epoch>:<sequence>` subscription cursors apart from the numeric paged read over the same bounded ring; caps on every dimension with typed busy before headers; explicit gap / history-unavailable frames that never fabricate and stay disjoint from delivered frames; release always, including on error and close; project a durable receipt only from the run's own recorded capability output (v1.195 P1 distilled; compound v1.195) |
| [architecture-patterns/hosted-compute-runtime-single-install.md](architecture-patterns/hosted-compute-runtime-single-install.md) | Hosted compute runtime — one engine, one warmed module cache and one `Semaphore(1)` serializer per owner (the engine's interrupt counter is engine-global, so a per-request serializer traps concurrent invocations at the shortest budget); install the three as ONE bundle only when every shipped embedded module warmed, because a partial warm makes the run path blame the caller with `not_found` for a module the registry still lists; with no bundle, discovery stays truthful and invocations refuse with the typed missing-runtime error (v1.195 P2 distilled; compound v1.195) |
| [workflow-patterns/single-live-action-request-guard.md](workflow-patterns/single-live-action-request-guard.md) | One authorized live action per attempt — a preload guard around the runtime's own global transport with the slot spent by filesystem exclusive-create **before** dispatch (atomic across children, consumed even on failure), one allowlisted origin/path/method with redirect forced to `error`, opaque forwarding with no credential access, fail-closed bookkeeping, deterministic fault controls before any live step, credential-shaped child vars stripped case-insensitively, missing credential reported as a named blocker distinct from a consumed failure, and runtime drift as a STOP (v1.195 P3 distilled; compound v1.195) |
| [architecture-patterns/tokio-notify-permit-steal-relay-fanout.md](architecture-patterns/tokio-notify-permit-steal-relay-fanout.md) | **Updated (v1.195)** — second lost-wakeup class appended: `notify_waiters` stores **no permit**, so a check-then-await waiter can park forever on a signal that already fired; the rule is register (`pin!` + `enable()`) → check → loop, with the publisher ordering state → flag → notify, and "check … then `.notified().await`" is a review finding |
| [conventions/pnpm-toolchain-pin-and-supply-chain-age.md](conventions/pnpm-toolchain-pin-and-supply-chain-age.md) | **Updated (v1.195)** — 2026-09-22 recurrence: the 720-minute window applies to freshly published first-party lockstep pins and is now handled as an explicit *delivery* gate (no PR, no CI-green claim before the cutoff; policy not edited for it; deferral registered), with implementation work remaining valid before the window closes |
| [engineering/spoke-lockstep-upgrade-procedure.md](engineering/spoke-lockstep-upgrade-procedure.md) | **Updated (v1.195)** — lockstep now covers a shared transport dependency: `spoke-connect` 0.14.1 requires `libp2p =0.57.0` and the feature-gated `connect-host` CLI shares libp2p values with that public API, so the companion pin moves in the same window; a schema-neutral/version-only upstream diff is still a multi-pin change; corrected the stale claim that CI is unaffected by the npm release-age window; added the dependency-only round's parity evidence shape |
| [engineering-conventions/canonical-invalid-input-422.md](engineering-conventions/canonical-invalid-input-422.md) | **Updated (v1.195)** — added the pre-filter hazard: a transport that silently drops unknown query keys before a strict decode turns a rejection into a confident wrong answer (an unsupported filter or typo returns a page that looks filtered), so unknown members are refused where the closed key set is known and the status is chosen deliberately (the canonical `invalid_input` mapping is 422, so a 400 there is a constructor choice) |
| [conventions/graph-pin-honesty-discipline.md](conventions/graph-pin-honesty-discipline.md) | **Updated (v1.195)** — removed the hardcoded libp2p version from its prose (the companion pin now moves with `spoke-connect`, so the doc and its probes state structure only, matching its own §5), and added the check-shape rule: a lib-only `cargo check -p <crate>` can fail while every CI job is green because a test build activates the crate's own dev-dependency, so name the shape in the obligation and run that shape |
| [architecture-patterns/connect-host-opt-in-feature-gate.md](architecture-patterns/connect-host-opt-in-feature-gate.md) | **Updated (v1.195)** — the N-C0 pattern keeps its feature-gate/manifest/refusal content, but the libp2p version is no longer restated: it moves with the `spoke-connect` pin because the feature-gated CLI shares libp2p identity types with that public API |
| [workflow-patterns/cargo-lockfile-feature-independent-dependabot.md](workflow-patterns/cargo-lockfile-feature-independent-dependabot.md) | **Updated (v1.195)** — the "alerts close only with libp2p ≥0.57 (unreleased)" unblock has since been adopted (`libp2p =0.57.0` in the v1.195 lockstep), so the earlier dispositions are stale inputs: re-run the four probes against the upgraded resolve, and note that a pin move is not a security review |
