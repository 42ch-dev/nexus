...
| **V1.75+** | **Canvas-pivot candidate** (retire V1.65 outline editor) + 8 QC suggestions (`tbd-v1.75-qc-followup`) + relationship confidence-weighting/filtering and relationship auto-extraction candidates. Desktop distribution v2 actual signing rollout remains blocked on Apple Developer ID cert + notarization credentials; Mobile (Tauri v2 mobile targets) remains future scope. |
