...
- **Body markdown is read-only** in V1.65/V1.66/V1.67 because orchestration already writes `body_path` through the host-tool path and there is no per-chapter edit lock yet. The V1.67 [body-editor.md](body-editor.md) Draft resolves this deferral with a reviewed per-chapter lock + conflict design; V1.68 implements the body-write route gated by that lock.
