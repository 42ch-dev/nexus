...
**`creator run` subcommands (V1.44):**

| Subcommand | Role | Shipped |
| --- | --- | --- |
| `review-master <work_id>` | Master-decision review on open findings; enqueues `novel-review-master` preset. Distinct from `stage advance --stage review` (which runs `reflection-loop` FL-E review stage). | V1.44 P1 |
| `audit-chapter <work_id>` | On-demand chapter audit (review/extract) outside auto-chain driver. | V1.44 P0 |

See [novel-quality-loop.md](novel-quality-loop.md) §3.4 for `review-master` flags and behavior; [novel-manuscript-audit.md](novel-manuscript-audit.md) for `audit-chapter`.
