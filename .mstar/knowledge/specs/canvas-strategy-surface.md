...
| Attribute | Value |
| --- | --- |
| **Status** | **Shipped β (V1.73)** — Strategy read + visualization + live overlay + Idea-steer (V1.70), write-boundary operation DTOs + node-granular Strategy edits + conflict policy (V1.71), Outline+Timeline canvas β (Work → Volume → Chapter → Scene/Beat graph projection + timeline lane + foreshadow edges + 3 structured patch routes `outline.patch_structure` / `outline.patch_chapter` / `timeline.patch_event` + outlineRevision + structured conflict error + UI retry/merge + non-spatial alternate views) (V1.72), and World KB canvas β (World KB graph + candidates projections, 2 structured patch routes `kb.patch_entity` / `kb.promote_candidate`, per-row OCC on `kb_key_blocks.revision` / `kb_extract_jobs.version`, 409/422 structured errors, and 4 Local API routes) (V1.73) are shipped; typed World KB relationship editing remains V1.74+ |
...
- **V1.74 candidate**: World KB relationships surface (`tbd-v1.74-world-kb-relationships`) — first-class typed relationship edges/CRUD beyond V1.73 source-anchor provenance projection.
- **V1.74+ candidate**: canvas-pivot (retire V1.65 outline whole-document editor in favor of node-granular canvas operations). Depends on V1.72 Outline+Timeline β maturity and post-V1.73 canvas adoption evidence.
