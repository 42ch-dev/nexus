...
| Attribute | Value |
| --- | --- |
| **Status** | **Shipped β (V1.72)** — Strategy read + visualization + live overlay + Idea-steer (V1.70), write-boundary operation DTOs + node-granular Strategy edits + conflict policy (V1.71), and Outline+Timeline canvas β (Work → Volume → Chapter → Scene/Beat graph projection + timeline lane + foreshadow edges + 3 structured patch routes `outline.patch_structure` / `outline.patch_chapter` / `timeline.patch_event` + outlineRevision + structured conflict error + UI retry/merge + non-spatial alternate views) (V1.72) are shipped; World KB canvas surface remains **Draft (V1.73+)** |
...
- **V1.73 lead (candidate)**: Canvas World KB surface (Draft §3.3 surface 3). Builds on entity-scope-model §5.5 promotion state machine. Likely `wire_contracts_changed: TRUE` for additive KB DTOs. Reuses Canvas Shell from V1.70 + Strategy β write patterns from V1.71 + Outline+Timeline β patterns from V1.72.
