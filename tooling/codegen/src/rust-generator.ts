 ...
 *   generated/{common,domain,platform/{http_bff,sync},daemon_api/compute}/<module>.rs
...
function generateRustModTree(schemas: LoadedSchema[], outputDir: string): void {
  ...
  const writeFolderMod = (node: FolderNode, relSegs: string[]) => {
    ...
    if (relSegs.length === 0) {
      ...
      lines.push('//!');
      lines.push('//! The root flat-globs every scope (`pub use domain::*`, `pub use daemon_api::*`, …).');
      lines.push('//! Scope submodule names can collide (e.g. `domain::memory` vs `daemon_api::memory`);');
      lines.push('//! the flat TYPE re-exports are all unique, so the module-name ambiguity is benign.');
      lines.push('#![allow(ambiguous_glob_reexports)]');
      lines.push('');
    }
