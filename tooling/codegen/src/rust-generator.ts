...
function generateRustModTree(schemas: LoadedSchema[], outputDir: string): void {
  ...
  const writeFolderMod = (node: FolderNode, relSegs: string[]) => {
    ...
    if (relSegs.length === 0) {
      ...
      lines.push('//!');
      lines.push('//! The root flat-globs every scope (`pub use domain::*`, `pub use local_api::*`, …).');
      lines.push('//! Scope submodule names can collide (e.g. `domain::memory` vs `local_api::memory`);');
