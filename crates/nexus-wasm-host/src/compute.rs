...
mod tests {
    ...
    fn resolve_sandbox_higher_than_host_clamped_to_default() {
        ...
        let manifest = make_manifest(
            ...
            Some(u64::try_from(crate::sandbox::DEFAULT_WALL_TIME.as_millis()).unwrap_or(u64::MAX) * 5), // 5× host default
        );
        let resolved = engine.resolve_sandbox(&manifest);
        assert_eq!(resolved.fuel, crate::sandbox::DEFAULT_FUEL);
        assert_eq!(
    ...
    fn resolve_sandbox_lower_than_host_manifest_wins() {
        ...
        let half_wall_ms = u64::try_from(crate::sandbox::DEFAULT_WALL_TIME.as_millis()).unwrap_or(u64::MAX) / 2;
        let manifest = make_manifest(
            "under-the-limit",
            Some(half_fuel),
            Some(half_mem_mib),
