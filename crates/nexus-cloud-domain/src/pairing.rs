...
impl Pairing {
    ...
    pub fn revoke(&mut self) -> Result<(), CloudDomainError> {
        if self.status != "active" {
            return Err(CloudDomainError::AlreadyInState(self.status.clone()));
        }
...
mod tests {
    ...
    fn test_revoke_already_revoked() {
        ...
        assert!(matches!(
            pairing.revoke(),
            Err(CloudDomainError::AlreadyInState(_))
        ));
    }
    ...
    fn revoke_rejects_non_active_status() {
        ...
        assert!(matches!(
            pairing.revoke(),
            Err(CloudDomainError::AlreadyInState(s)) if s == "pending_review"
        ));
    }
}
