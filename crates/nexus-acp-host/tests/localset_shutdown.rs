...
use nexus_acp_host::localset_bridge::{
    LocalSetBridge, MAX_ACTIVE_TASKS, MAX_PENDING_REQUESTS, SHUTDOWN_JOIN_BUDGET,
};
...
async fn localset_shutdown() {
    for cycle in 0..100 {
        ...
        let n: i32 = bridge
            .execute(move || Box::pin(async move { cycle }))
            .await
...

    {
        let bridge = LocalSetBridge::new();
        let _ = bridge
            .execute(|| Box::pin(async { tokio::time::sleep(Duration::from_millis(20)).await }))
            .await;
        let b1 = bridge.clone();
        let b2 = bridge.clone();
...

    {
        ...
        let hang = tokio::spawn(async move {
            let _ = b
                .execute(|| Box::pin(async { std::future::pending::<()>().await }))
                .await;
        });
...

    {
        ...
        for _ in 0..MAX_ACTIVE_TASKS {
            ...
            handles.push(tokio::spawn(async move {
                let _ = b
                    .execute(|| Box::pin(async { tokio::time::sleep(Duration::from_secs(5)).await }))
                    .await;
            }));
        }
        tokio::time::sleep(Duration::from_millis(50)).await;
        let busy = bridge
            .execute(|| Box::pin(async { 99 }))
            .await;
