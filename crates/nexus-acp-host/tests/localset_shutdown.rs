...
#[tokio::test]
async fn execute_shutdown_cycles_settle_repeatedly() {
    for cycle in 0..100 {
...
#[tokio::test]
async fn cancelled_waiter_aborts_its_tracked_task() {
    let bridge = LocalSetBridge::new();
...
#[tokio::test]
async fn shutdown_bypasses_a_full_work_queue() {
    let bridge = LocalSetBridge::new();
    ...
    assert!(
        ...
        "shutdown bypassed full queue in {elapsed:?}, evidence={evidence:?}"
    );
...
#[tokio::test]
async fn concurrent_shutdowns_share_one_settlement() {
    let bridge = LocalSetBridge::new();
    let _ = bridge
        .execute(1, || {
...
#[tokio::test]
async fn shutdown_aborts_hung_task_and_settles() {
    let bridge = LocalSetBridge::new();
    let b = bridge.clone();
...
#[tokio::test]
async fn active_cap_saturation_defers_extra_work() {
    let bridge = LocalSetBridge::new();
    let bridge_ref = bridge.clone();
...
#[tokio::test]
async fn oversize_payload_is_rejected_at_admission() {
    let bridge = LocalSetBridge::new();
...
#[tokio::test]
async fn multibyte_oversize_payload_fails_admission() {
    let bridge = LocalSetBridge::new();
...
#[tokio::test]
async fn shutdown_records_aborted_task_ids() {
    let bridge = LocalSetBridge::new();
...
#[tokio::test]
async fn deferred_work_never_exceeds_active_cap() {
    let bridge = LocalSetBridge::new();
    let bridge_ref = bridge.clone();
    let mut handles = Vec::new();
...
#[tokio::test]
async fn pre_poll_cancel_releases_active_slot_once() {
...
#[tokio::test]
async fn control_work_bypasses_saturated_queue() {
...
#[tokio::test]
async fn pending_queue_fills_independently_of_active_cap() {
...
#[tokio::test]
async fn bounded_join_retains_handle_for_a_later_join() {
    ...
    assert!(
        !evidence.aborted_task_ids.is_empty() || evidence.active_at_shutdown >= 1,
