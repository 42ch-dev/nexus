...
async fn annotation_create_list_patch_delete_round_trip() {
    ...
    let updated = core
        .patch_annotation(&principal, created.annotation_id.clone(), patch)
        .await
        .unwrap();
    assert_eq!(updated.color.to_string(), "blue");
    assert_eq!(updated.note, Some("note".to_string()));

    core.delete_annotation(&principal, created.annotation_id.clone())
        .await
        .unwrap();
...
async fn annotation_note_clear_and_scope_isolation() {
    ...
    let cleared = core
        .patch_annotation(&principal, created.annotation_id.clone(), clear)
        .await
        .unwrap();
...
async fn findings_list_pagination_and_filter_parity() {
    ...
    let Err(CoreError::InvalidInput { reason, .. }) = core
        .list_findings(
            &principal,
            work_id,
            nexus_core::ListFindingsQuery {
...
async fn findings_update_and_batch_triage_semantics() {
    ...
    let first = core
        .create_finding(&principal, work_id.clone(), make("one"))
        .await
        .unwrap();
    let second = core
        .create_finding(&principal, work_id.clone(), make("two"))
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        ("invalid_input", "finding_ids must not be empty")
    );
    ...
    assert_eq!(
        ...
        (
            "too_many_findings",
            "batch update is capped at 100 findings; received 101"
        )
    );
    ...
    let no_patch = core
        .batch_update_findings(
            &principal,
            BatchUpdateFindingsRequest {
                finding_ids: vec![first.finding_id.clone()],
                patch: Default::default(),
            },
        )
        .await
    ...
    .await
    .unwrap_err(); // resolved is terminal — rejected as invalid_transition
    let report = core
        .batch_update_findings(
            &principal,
            BatchUpdateFindingsRequest {
    ...
    core.delete_finding(&principal, first.finding_id.clone())
        .await
        .unwrap();
    assert!(matches!(
        core.delete_finding(&principal, first.finding_id.clone())
...
async fn kb_scope_isolation_and_entry_round_trip() {
    ...
    let Err(CoreError::InvalidInput { field, reason }) = core
        .list_kb_entries(
            &principal,
            ListKbEntriesQuery {
                creator_id: Some("author".into()),
                workspace_slug: None,
                scope: Some("world".into()),
                q: None,
                limit: None,
                cursor: None,
            },
    ...
    assert!(
        reason.contains("entity-scope-model"),
        "scope error must cite entity-scope-model, got: {reason}"
    );
    assert!(
        reason.contains("work-scope file index only"),
        "scope error must explain work-scope only, got: {reason}"
    );
    ...
    for slug in ["../escape", "a\\b", "..", "/abs", ".", ""] {
        let Err(CoreError::InvalidInput { field, .. }) = core
            .list_kb_entries(
                &principal,
                ListKbEntriesQuery {
                    creator_id: Some("author".into()),
                    workspace_slug: Some(slug.into()),
                    scope: None,
                    q: None,
                    limit: None,
                    cursor: None,
    ...
    let added = core
        .add_kb_entry(
            &principal,
            nexus_contracts::daemon_api::kb::AddKbEntryRequest {
                content: Some("# note body".into()),
    ...
    let got = core
        .get_kb_entry(&principal, added.entry_id.clone())
        .await
        .unwrap();
    ...
    let deleted = core
        .delete_kb_entry(&principal, added.entry_id.clone())
        .await
        .unwrap();
    ...
    let Err(CoreError::InvalidInput { field, .. }) = core
        .get_kb_entry(&principal, "kb_../../etc/passwd".into())
        .await
    else {
...
async fn create_finding_rejects_foreign_or_missing_work() {
    ...
    assert!(matches!(
        core.create_finding(&principal, work_id.clone(), make("t"))
            .await,
        Err(CoreError::NotFound { .. })
    ));
    core.close().await.unwrap();
...
async fn get_work_finding_binds_row_to_path_work() {
    ...
    let world = core
        .create_world(
            ...
            serde_json::from_value::<CreateWorldRequest>(
                serde_json::json!({"title": "Second World"}),
            )
            .unwrap(),
        )
        .await
    ...
    assert_eq!(
        core.get_work_finding(&principal, work_a.clone(), finding.finding_id.clone())
            .await
            .unwrap()
            .finding_id,
        finding.finding_id
