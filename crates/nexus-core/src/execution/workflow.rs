...
impl WorkflowRunCoordinator {
    ...
    async fn prepare_legacy_row(
        ...
        let loaded = nexus_preset::resolve_preset(&row.preset_id, nexus_home, &registry)
...
#[cfg(test)]
mod tests {
    // The tests were written against the module's own scope (the daemon's
    // `preset_run` test module) and exercise private internals, so they see the
    // parent items directly. `super::*` re-supplies the engine/storage/trait
    // imports and the public and private items alike.
    use super::*;
    use async_trait::async_trait;
    ...
    /// Seed an admitted engine-owner workspace DB, mirroring the daemon's own
    /// `create_test_workspace` fixture (same admission path, so the
    /// writer-protocol guards admit the test's writes). This replaced the
    /// daemon fixture when the driver moved to core.
    async fn fixture() -> (tempfile::TempDir, std::path::PathBuf, std::path::PathBuf) {
        let tmp = tempfile::tempdir().expect("tempdir");
        let user_home = tmp.path().to_path_buf();
        let nexus_home = user_home.join(".nexus42");
        std::fs::create_dir_all(&nexus_home).expect("nexus_home dir");
        std::fs::write(
...
\"test_creator\" = \"default\"",
    ...
    async fn failed_drive_persists_failure_record_before_terminal_settlement() {
        ...
        let (tmp, _nexus_home, db_path) = fixture().await;
        let _ = &tmp;
        let guarded = nexus_local_db::init_engine_pool(&db_path)
            .await
            .expect("open pool");
    ...
    #[async_trait]
    impl RunEventPort for FullQuotaPort {
        async fn try_register_live(&self, _run_id: &str) -> bool {
            false
        }
        async fn remove_live(&self, _run_id: &str) {}
    ...
    /// now expressed against the port): the per-run live event-ring quota is
    /// RESERVED before any drive spawn. A run that cannot reserve is refused
    /// with the TYPED capacity error, no drive owner is registered, and the
    /// durable existing work is untouched. The ring registry itself is the
    /// transport's; its own quota/handoff tests live in
    /// `nexus-daemon-runtime`'s `run_events` module.
    #[tokio::test]
    ...
    async fn ensure_driving_refuses_capacity_without_touching_durable_work() {
        ...
        .with_run_events(Arc::new(FullQuotaPort));
        ...
        let err = coordinator
            .ensure_driving(&session_id)
            .await
            .expect_err("a run that cannot reserve a live ring must be refused");
        assert!(
            matches!(&err, RunControlError::RunEventCapacity(sid) if sid == &session_id.0),
            "capacity refusal must be typed, got {err:?}"
        );

        // No drive owner was registered for the refused run.
        assert!(
    ...
    /// A port double that records what the coordinator published and whether
    /// it closed the ring as authoritative terminal.
    #[derive(Default)]
    struct RecordingPort {
        published: parking_lot::Mutex<Vec<(String, String)>>,
        terminal: parking_lot::Mutex<Vec<String>>,
    }

    #[async_trait]
    impl RunEventPort for RecordingPort {
        async fn try_register_live(&self, _run_id: &str) -> bool {
            true
        }
        async fn remove_live(&self, _run_id: &str) {}
        fn publish_run_state(&self, run_id: &str, record: &RunRecord) {
            self.published
                .lock()
                .push((run_id.to_string(), record.status.as_db_str().to_string()));
        }
        fn mark_terminal(&self, run_id: &str) {
    ...
    /// actionable "cleanup unconfirmed" outcome, so its ring stays live for a
    /// retry cancel to publish the confirmed winner on the SAME ring.
    #[tokio::test]
    async fn interrupted_run_keeps_ring_live_until_retry_cancel_confirms() {
        ...
        .with_run_events(Arc::clone(&port) as Arc<dyn RunEventPort>);
        ...
        // Durable winner: cleanup unconfirmed → Interrupted + cancel intent.
        let record = store
            .load_run(&session_id)
            .await
            .expect("load")
        ...
        assert_eq!(
            port.published.lock().last().cloned(),
            Some((session_id.0.clone(), "interrupted".to_string())),
            "unconfirmed cleanup must be published as interrupted"
        );
        assert!(
            port.terminal.lock().is_empty(),
            "an interrupted run must NOT be closed as authoritative terminal"
        );

        // Retry cancel confirms: durable winner = Cancelled.
        let record = store
            .load_run(&session_id)
            .await
            .expect("load")
        ...
        coordinator.publish_durable_run_state(&session_id).await;
        assert_eq!(
            port.terminal.lock().last().cloned(),
            Some(session_id.0.clone()),
            "the confirmed terminal winner must close the ring"
        );
        let durable = store
            .load_run(&session_id)
            .await
        ...
        assert_eq!(
            port.published.lock().last().cloned(),
            Some((session_id.0.clone(), durable.status.as_db_str().to_string())),
            "the final published status must match the durable DB status"
        );
    }
}
