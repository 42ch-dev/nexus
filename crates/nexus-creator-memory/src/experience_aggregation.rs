...
pub trait ExperienceSynthesizer: Send + Sync {
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<String, MemoryError>> + Send + '_>>;
}
...
mod tests {
    ...
    async fn aggregate_experience_with_synthesizer_success() {
        ...
        impl ExperienceSynthesizer for MockSynthesizer {
            fn synthesize(
                ...
                Box<dyn std::future::Future<Output = Result<String, MemoryError>> + Send + '_>,
            > {
                let result = format!(
    ...
    async fn aggregate_experience_synthesizer_failure_falls_back() {
        ...
        impl ExperienceSynthesizer for FailingSynthesizer {
            ...
            > {
                Box::pin(
                    async move { Err(MemoryError::ValidationError("ACP unavailable".to_string())) },
                )
            }
        }
    ...
    // Note: Stage0Assembly E2E tests moved to nexus-moment-context-assembly (Batch C).
    // Remaining E2E tests only use experience_aggregation + soul + memory_io.
