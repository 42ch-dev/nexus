...
//!
//! V1.148 P3 N-C0: this module moved here from `nexus-cloud-sync` so the
//! path-layout SSOT (this crate owns [`crate::device_id_path`]) also owns the
//! value read/write. `nexus-cloud-sync::device_id` re-exports these items, so
//! the `nexus_cloud_sync::device_id::get_or_create_device_id` call site in
//! `apps/nexus42` is unchanged.
...
mod tests {
    ...
    fn first_call_creates_device_id_with_valid_uuid() {
        ...
        let path = crate::device_id_path(tmp.path());
    ...
    fn parent_directory_created_automatically() {
        ...
        let path = crate::device_id_path(&deep);
        assert!(
            path.exists(),
