...
//!
//! Provides async connection pooling using deadpool-sqlite, matching the
//! daemon's pool.rs implementation pattern.
//!
//! # Usage
//!
//! ```ignore
...
//!
//! let pool = OutboxPool::new(&db_path, DEFAULT_POOL_SIZE)?;
...
///
/// Provides async connection retrieval for concurrent outbox access.
...
impl PooledConn {
    ...
    ///
    /// Useful for schema initialization and migrations.
...
mod tests {
    ...
    fn pool_creates_successfully() {
        ...
        let pool = OutboxPool::new(&db_path, 2).expect("Pool creation should succeed");
    ...
    async fn pool_get_returns_working_connection() {
        ...
        let pool = OutboxPool::new(&db_path, 2).unwrap();
    ...
    async fn pool_supports_concurrent_access() {
        ...
        let pool = OutboxPool::new(&db_path, 4).unwrap();
