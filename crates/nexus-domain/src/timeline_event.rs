...
impl TimelineEvent {
        ...
        if let Some(ref causes) = self.caused_by_event_ids {
            for cause_id in causes {
                if cause_id == &self.timeline_event_id {
                    return Err(DomainError::CausalityViolation(
    ...
    pub fn reject(&mut self) -> Result<(), DomainError> {
        if self.status == TimelineEventStatus::Rejected.as_str() {
            return Err(DomainError::AlreadyInState("rejected".to_string()));
        }
    ...
    pub fn validate_causality(&self, world_id: &str) -> Result<(), DomainError> {
        ...
        if self.world_id != world_id {
            return Err(DomainError::CausalityViolation(format!(
...
mod tests {
    ...
    fn test_self_causality_rejected() {
        ...
        assert!(matches!(
            ...
            Err(DomainError::CausalityViolation(_))
        ));
    }
