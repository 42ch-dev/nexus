...
//! a graph-free `LoadedPreset` for authoring and execution consumers.
//!
...
use crate::source_identity::PresetSourceIdentity;
...
/// A validated preset manifest and its source metadata.
///
/// Design: `orchestration-engine.md` §8.1.
#[derive(Debug, Clone)]
pub struct LoadedPreset {
    ...
    pub signals: Vec<crate::manifest::SignalBinding>,
    ...
    pub initial_action: Option<crate::manifest::InitialAction>,
    /// Per-state context update hooks (keyed by state ID).
    pub context_update_hooks: HashMap<String, crate::manifest::ContextUpdateHook>,
    /// Role definitions for multi-agent presets (WS-E T6).
    /// Empty = single-agent mode (backward compatible).
    pub roles: Vec<crate::manifest::PresetRoleDefinition>,
...
///
/// # Errors
/// Returns [`PresetLoadError`] if YAML parsing or validation fails.
...
pub fn preset_source_identity(
    ...
    for rel in &asset_paths {
        ...
        let bytes = if let Some(root) = &canonical_root {
            ...
            crate::read_embedded_template(id, rel)
...
fn validate_manifest(
    ...
    for (i, state) in manifest.states.iter().enumerate() {
        ...
        for (j, enter) in state.enter.iter().enumerate() {
            ...
            match enter {
                crate::manifest::EnterAction::Capability { name, .. } => {
                    if caps.input_schema(name).is_none() {
                        problems.push(ValidationProblem {
                ...
                crate::manifest::EnterAction::InnerGraph { name } => {
                ...
                crate::manifest::EnterAction::HostTool { .. } => {
        ...
        {
            if caps.input_schema(cap_name).is_none() {
                problems.push(ValidationProblem {
        ...
        if let Some(ref hook) = state.context_update {
            match &hook.op {
                ...
                ContextUpdateOp::LlmSummarize { capability } => {
                    ...
                    if caps.input_schema(capability).is_none() {
                        problems.push(ValidationProblem {
...
mod tests {
    ...
    fn test_capability_registry() -> BuiltinCapabilityCatalog {
...
";
    ...
    fn parse_initial_action_and_context_update() {
        use crate::manifest::{ContextUpdateOp, InitialAction};
...
";
    ...
    fn parse_initial_action_seed_expansion() {
        use crate::manifest::InitialAction;
...
"#;
    ...
    fn context_update_struct_merge_parses() {
        use crate::manifest::ContextUpdateOp;
