...
use nexus_moment_context_assembly::{MomentContext, MomentRequest, assemble_moment};
...
async fn seed_kb_block(stores: &FixtureStores, world_id: &str, block_type: BlockType, name: &str) {
    ...
    stores.kb.insert_key_block(kb).await.expect("insert key block");
}

/// Seed a knowledge entry.
async fn seed_knowledge(stores: &FixtureStores, user_id: &str, tags: &[&str], content: &str) {
    let tag_vec: Vec<nexus_knowledge::KnowledgeTag> =
        tags.iter().map(|t| nexus_knowledge::KnowledgeTag::new(t)).collect();
    let entry = nexus_knowledge::KnowledgeEntry::new(user_id, tag_vec, content);
    stores.knowledge.store(entry).await.expect("store knowledge");
}
...
async fn narrative_context_provides_world_and_timeline() {
    ...
    assert!(ws.contains("Narnia Chronicles"), "world state must include title");
    assert!(ws.contains("wld_narnia"), "world state must include world_id");

    // Timeline
    let tl = ctx.timeline.expect("timeline must be present");
    assert!(tl.contains("The Wardrobe"), "timeline must include first event");
    assert!(tl.contains("The White Witch"), "timeline must include second event");
}
...
async fn world_kb_assets_are_included_by_block_type() {
    ...
    seed_kb_block(&stores, "wld_rpg", BlockType::InfoPoint, "The Great Cataclysm").await;
    ...
    assert!(kb_text.contains("Aria the Mage"), "KB must list character block");
    assert!(kb_text.contains("Crystal Tower"), "KB must list location block");
    assert!(kb_text.contains("The Great Cataclysm"), "KB must list lore block");
}

#[tokio::test]
async fn user_knowledge_entries_are_included_with_tags() {
    let stores = FixtureStores::empty();
    seed_knowledge(&stores, "user_42", &["worldbuilding", "magic"], "Magic requires a price.").await;
    seed_knowledge(&stores, "user_42", &["character"], "Villains need motivations.").await;
    ...
    assert!(uk_text.contains("Magic requires a price"), "must include first entry");
    assert!(uk_text.contains("Villains need motivations"), "must include second entry");
    assert!(
...
async fn full_assembly_produces_correct_moment_context() {
    ...
    assert!(ctx.user_knowledge.is_some(), "user_knowledge must be populated");

    // Full context combines everything
    let full = ctx.to_full_context();
    assert!(full.contains("speculative fiction writer"), "full must have personality");
    assert!(full.contains("The Great Epic"), "full must have world state");
    assert!(full.contains("The Call to Adventure"), "full must have timeline");
    assert!(full.contains("Protagonist"), "full must have KB");
    assert!(full.contains("Use active voice"), "full must have knowledge");
}
