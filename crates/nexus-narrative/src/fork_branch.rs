...
impl ForkBranch {
    ...
    pub fn request_verification(&mut self) -> Result<(), NarrativeError> {
        ...
        if self.verification_status != VerificationStatus::Unverified.as_str() {
            return Err(NarrativeError::InvalidTransition {
    ...
    pub fn verify(&mut self) -> Result<(), NarrativeError> {
    ...
    pub fn reject(&mut self, _reason: &str) -> Result<(), NarrativeError> {
        if self.verification_status != VerificationStatus::Requested.as_str() {
            return Err(NarrativeError::InvalidTransition {
                from: self.verification_status.clone(),
    ...
    pub fn validate_write_scope(&self, target_world_id: &str) -> Result<(), NarrativeError> {
        if self.status != ForkBranchStatus::Active.as_str() {
            return Err(NarrativeError::InvalidForkWriteScope(
                "fork branch is not active".to_string(),
            ));
        }
        if target_world_id != self.world_id {
            return Err(NarrativeError::InvalidForkWriteScope(format!(
...
mod tests {
    ...
    fn test_verify_without_request() {
        ...
        assert!(matches!(
            fb.verify(),
            Err(NarrativeError::InvalidTransition { .. })
        ));
    }
    ...
    fn test_write_scope_validation_parent() {
        ...
        assert!(matches!(
            ...
            Err(NarrativeError::InvalidForkWriteScope(_))
        ));
    }
    ...
    fn request_verification_fails_when_archived() {
        ...
        assert!(matches!(
            ...
            Err(NarrativeError::InvalidState { expected, actual })
    ...
    fn request_verification_second_call_invalid_transition() {
        ...
        assert!(matches!(
            fb.request_verification(),
            Err(NarrativeError::InvalidTransition { from, to })
    ...
    fn validate_write_scope_fails_when_archived() {
        ...
        assert!(matches!(
            ...
            Err(NarrativeError::InvalidForkWriteScope(msg)) if msg.contains("not active")
        ));
    }
}
