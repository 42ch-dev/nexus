...
async fn with_runtime_deps_registers_all_llm_capabilities() {
    ...
    // Same 23 builtins as with_builtins() + essay.scaffold from V1.52 T-A P2 + game_bible.scaffold from V1.54 P1 —
    // drop-in replacement for the production registry construction.
    assert_eq!(
        registry.len(),
        23,
        "registry should have 23 builtins (21 V1.51 + 1 essay.scaffold from V1.52 T-A P2 + 1 game_bible.scaffold from V1.54 P1)"
    );
