...
pub fn validate_hash_hex(hash: &str) -> Result<(), SessionError> {
    if hash.len() != MAX_HASH_LEN {
        return Err(SessionError::ManifestInvalid("expectedHash must be 64-char lowercase hex".into()));
    }
    if !hash.chars().all(|c| matches!(c, '0'..='9' | 'a'..='f')) {
        return Err(SessionError::ManifestInvalid("expectedHash must be lowercase hex".into()));
    }
    Ok(())
}
...
pub fn validate_stage_basename(name: &str) -> Result<(), SessionError> {
    if name.is_empty() || name.len() > MAX_BASENAME_LEN {
        return Err(SessionError::ManifestInvalid("invalid stage basename".into()));
    }
...
pub fn validate_intent_op(op: &str) -> Result<(), SessionError> {
    match op {
        ...
        _ => Err(SessionError::ManifestInvalid(format!("invalid intent op: {op}"))),
    }
}
