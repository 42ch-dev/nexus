...
| Connect SDK | `@42ch/spoke-connect@0.13.0` (npm) | Your backend's connection + invoke surface to the runtime (and to any SPOKE connect host). |
...
3. Start the runtime; the readiness line is `nexus-runtime: Connect Host
   (N-C2 E2) ready`.
4. Install the Connect SDK: `npm install @42ch/spoke-connect@0.13.0`, and
...
canonical name — not `spoke-connect-ts`), pinned exactly to **0.13.0**:

```bash
npm install @42ch/spoke-connect@0.13.0
```
...
- Connect SDK + wire family: `@42ch/spoke-connect@0.13.0` on npm
